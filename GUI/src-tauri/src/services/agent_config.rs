...
impl AgentConfigService {
    ...
    pub fn ensure_butler(&self) -> Result<(), AppError> {
        ...
        let agents = config
            .as_object_mut()
            .and_then(|o| o.entry("agent").or_insert_with(|| json!({})).as_object_mut());
    ...
    pub fn full_sync(&self, roles: &[Role]) -> Result<(), AppError> {
        ...
        let existing_butler = root
            .get("agent")
            .and_then(|a| a.get(BUTLER_KEY))
            .cloned();
        agents.insert(
            BUTLER_KEY.to_string(),
            existing_butler.unwrap_or_else(|| {
                json!({
                    "name": "管家",
                    "mode": "primary",
                    "prompt": "你是EgoSync管家",
