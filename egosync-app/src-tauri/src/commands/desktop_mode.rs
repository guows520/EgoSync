...
use crate::services::desktop_mode::{self, DesktopModeFile};
...
#[tauri::command]
pub fn desktop_get_boot_config(app: AppHandle) -> Result<DesktopBootConfig, AppError> {
    ...
    let mode = file
        .as_ref()
        .map(|f| desktop_mode::parse_mode(&f.mode))
...
#[tauri::command]
pub fn remote_mode_save_config(
    app: AppHandle,
    mode: String,
    ...
    let parsed = desktop_mode::parse_mode(&mode);
    // 解析结果与请求 mode 不一致 ⇒ 未知 mode 值（parse 回退 local）——
    // 显式拒绝而非静默落 local（写路径与读路径 fail-safe 语义不同：
    // 写错方向比读错方向危险）
    ...
    // remote：URL 必填 + scheme 校验；token 提供即写 keyring
    if parsed.is_remote() {
        let url = remote_url
            .as_deref()
            .map(str::trim)
        ...
        desktop_mode::validate_remote_url(url)?;
        if let Some(token) = token.as_deref() {
            ...
            desktop_mode::save_remote_token(trimmed)?;
        }
    }

    desktop_mode::write_mode_file(
        &app_data_dir,
        &DesktopModeFile {
