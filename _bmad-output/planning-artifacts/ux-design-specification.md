...
> **2026-09-25 修订注记（云端版立项后升级）**：云端托管版（PRD §4.15）立项后，移动浏览器为 WEB 客户端第一类客户端（FR-45），原「手机 <768px = V2 考虑」升级为正式范围——落位于文末新增章节「云端版 WEB 移动形态」（Story 16.4，2026-09-25 Correct Course 批准）。本表原文保留不回改，以本注记与新增章节为准。
...
## 云端版 WEB 移动形态（2026-09-25 新增章节）

> 本章即 Story 16.2 延期的「UX 定稿级移动形态」勘定，结清 UX-C4「断点与移动交互细节由 UX 阶段确定」欠账。依据：`sprint-change-proposal-2026-09-25.md`（已批准）；架构「⑩移动端适配与 PWA」决策块；PRD FR-45 补条。边界：与桌面版同一 React 组件体系、视觉零分叉（UX-C1）——本章只定义小屏形态收敛与 PWA 交互，不新造视觉。

### 断点与导航

- **断点**：`768px`（CSS 逻辑像素）——沿用本规范 Responsive Strategy 既有断点体系，不新增断点。
- **<768px**：左侧 56px 侧栏退场，改**底部 tab 导航**：`管家 / 角色 / 设置`（初案，允许实现期微调文案与顺序；tab 数量/位置等结构变更须回报产品负责人）。主区域全屏。
- **≥768px**：桌面布局逐像素不变（硬边界——`md:` 桌面值 + `max-md:` 移动值双声明模式）。
- 角色详情视图内的对话 / 任务 / 记忆既有 tab 结构在小屏保持（顶部 tab，可横向滚动）；仪表盘 = 管家视图内既有 tab，不新设入口。

### 五大核心面的小屏形态

| 面 | <768px 形态 | 锚点 |
|---|---|---|
| 管家对话 | 全屏对话；输入区贴底 + 安全区避让；虚拟键盘弹出不遮发送按钮 | 沿用规范既有「全屏对话布局」要点 |
| 管家摘要 | 宽度 100%，padding 16px | 同上 |
| ActionCard | 全宽堆叠 | 同上 |
| 任务 | 四象限分组列表单列；拖拽排序在触屏降级为「移动模式」入口（排序按钮）或长按拖拽——实现期定，禁止触屏上拖拽失效且无替代路径 | 新增（触屏对拖拽的固有约束） |
| 仪表盘 | 网格降列（16.2 已交付的塌缩基线） | 16.2 既有落地 |

### 触控与密度

- **触控目标 ≥44×44px**（本规范既有 Implementation Guidelines 条目，移动端逐面走查强制执行；视觉上更小也要保点击区域）。
- **密度双模式对齐 companion-android**（`spec-companion-android-design-language-parity`）：对话屏轻密度 / 仪表盘密密度——WEB 端以既有桌面值为锚做小屏收敛（padding/间距按既有 token 缩档），不新造密度值。
- 动效不变：呼吸（3s）/ 色温 300ms / 思考点等既有动效移动端原样；`prefers-reduced-motion` 静态降级已支持（web 平台等价物）。

### 安全区（iOS）

- `viewport-fit=cover` 已由 16.2 开启；100vh 地址栏抖动缓解（dvh 优先 + 回退）保持不回退。
- 底部 tab 栏、对话输入区、模态、Toast 统一以 `env(safe-area-inset-bottom)` 等避让刘海/底部横条；standalone（加主屏）模式同样生效。

### PWA 安装体验

- **不加侵入式安装弹窗**：依赖浏览器原生「添加到主屏幕」+ manifest 元数据（单用户自托管产品气质——克制优先）。
- **图标与启动画面**：maskable 图标（192/512）+ theme-color；standalone 启动即品牌色，无额外启动画面造景（零新视觉资产）。
- **离线语义如实呈现**：外壳离线可用 = 界面可打开；业务数据仍需从服务端重取（刷新=重取语义不变）——断线时连接状态条（16.2 既有三态徽标）即诚实明示，不为离线假装有数据。
- **推送不做**（PRD Non-Goal）：iOS Safari 16.4+ 仅限加主屏 PWA 支持 Web Push——维持「WEB 端应用内通知」裁决，本章不引入推送交互。
