...
describe('HttpTransport 远程桌面通道', () => {
  ...
  it('退避 governor：1s→2s→4s…封顶 30s + ±20% 抖动', async () => {
    vi.useFakeTimers();
    const randomSpy = vi.spyOn(Math, 'random').mockReturnValue(0.5); // 抖动中心（无偏移）
    try {
      const transport = makeRemote();
      // 驱动连续致命失败：签发成功建流后 onerror（重试路径一次）+ 退避循环
      transport.on(SAMPLE_EVENT, () => {});
      await vi.advanceTimersByTimeAsync(0);
      expect(FakeEventSource.instances).toHaveLength(1);
      FakeEventSource.instances[0]!.onopen!();

      // 失败轮次 1：立即重取重试（票据重试一次）——也失败 ⇒ 退避 1s 档
      ticketResponse = () => jsonResponse(500, { error: 'boom' });
      FakeEventSource.instances[0]!.onerror!();
      await vi.advanceTimersByTimeAsync(0);
      await vi.advanceTimersByTimeAsync(0);
      // 重取票据失败 ⇒ scheduleRebuild（attempt=1 ⇒ ~1s）
      expect(transport['rebuildTimer']).not.toBeNull();

      const delays: number[] = [];
      // 连续失败循环：每次退避到期重试（签发 500）⇒ 再退避（attempt 递增）
      for (let i = 0; i < 5; i++) {
        const before = fetchCalls.filter(c => c.url.endsWith('/api/events/ticket')).length;
        await vi.advanceTimersByTimeAsync(31_000); // 足够走完一次退避窗口
        const after = fetchCalls.filter(c => c.url.endsWith('/api/events/ticket')).length;
        // 退避到期必须重试
