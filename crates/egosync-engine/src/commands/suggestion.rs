//! suggestion 域命令体（Story 15.4 自壳 `commands/suggestion.rs` 平移，
//! 业务逻辑零改动；State 取值改 `&EngineCtx`；内联测试随迁）。
//!
//! `confirm_and_create_task` / `reject_with_reason` 保持 pub——桌面壳
//! companion_dispatch 复用唯一入口（Story 13.3 受控例外 A），经壳回引。
...
pub async fn suggestion_list_pending(
    ...
    suggestions::list_pending_suggestions(&ctx.pool, &conversation_id).await
}

pub async fn suggestion_confirm(ctx: &EngineCtx, id: String) -> Result<Suggestion, AppError> {
    confirm_and_create_task(&ctx.pool, &id).await
}
...
pub async fn suggestion_reject(
    ...
    reject_with_reason(&ctx.pool, &id, &reason).await
}
