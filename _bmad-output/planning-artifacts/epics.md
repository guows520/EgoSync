...
user_name: boss
updated: 2026-07-22
incrementalScope: [FR-37, FR-38, FR-39, FR-40, FR-41, FR-42, FR-43, FR-44, FR-45, FR-46]
incrementalStepsCompleted: [1, 2, 3, 4]
---
...
## Incremental Requirements Inventory: FR-37–FR-46

> 本增量仅拆分 FR-37 至 FR-46。既有 FR-1 至 FR-36、Epic 1–8 及其 Stories 保持不变。

### Incremental Functional Requirements

- **FR-37**：管家与角色支持通过 EgoSync 提供的 `@Skill` 选择器显式指定当前消息使用的 Skill；底层映射到 OpenCode 原生 Skill 加载机制，且不可用 Skill 不得执行并需明确反馈。
- **FR-38**：管家仪表盘展示任务总数、未完成任务数、记忆总数和会话数；默认汇总全部活跃角色并支持按单角色筛选。
- **FR-39**：管家设置分组支持独立及全部折叠/展开，并支持启用或关闭 MCP Server。
- **FR-40**：角色设置分组支持折叠/展开；自定义 Skill 归入 Skill 配置；统一 MCP Server配置命名；角色信息在分组内独立保存。
- **FR-41**：创建任务时，日期、小时、分钟选择完整后自动关闭时间弹窗并回填，可重新打开修改。
- **FR-42**：新增和编辑角色时支持填写、保存及查看目标；对话提炼目标可回填确认。
- **FR-43**：对话及富文本区域正确渲染标准 Markdown 表格，支持宽表格防溢出和双主题。
- **FR-44**：用户可见文案统一将“分身管家”改为“数字分身管家”。
- **FR-45**：建议对话框及右上角选项标签补齐深色模式的默认、悬停、选中和禁用状态。
- **FR-46**：全新安装和首次启动默认中文，不覆盖既有用户语言选择。

### Incremental Non-Functional Requirements

- **NFR-INC-1：即时生效** — Skill 和 MCP Server 状态变更后无需重启应用。
- **NFR-INC-2：能力边界安全** — 已关闭或不可用的 Skill/MCP Server 不得被调用，并提供明确反馈。
- **NFR-INC-3：统计一致性** — 四项统计必须共享同一角色筛选范围，清除筛选后恢复全部活跃角色汇总。
- **NFR-INC-4：布局稳定性** — 宽 Markdown 表格不得破坏消息布局，使用横向滚动或等价防溢出处理。
- **NFR-INC-5：主题可读性** — Markdown 表格、建议对话框和导航标签在浅色与深色模式下均可读。
- **NFR-INC-6：保存隔离性** — 角色信息保存失败不得影响其他设置分组状态，并显示明确反馈。
- **NFR-INC-7：升级兼容性** — 默认中文仅应用于全新安装或无语言偏好用户，升级不得覆盖已有选择。

### Incremental Architecture Requirements

- **AR-INC-1**：Skill 和 MCP Server 配置通过 Rust `agent_config` 层同步至 opencode，不由前端直接修改 `opencode.json`。
- **AR-INC-2**：统计、角色信息和配置操作继续通过 Tauri IPC 调用 Rust 后端；前端不直接访问数据库或 opencode Server。
- **AR-INC-3**：任务与记忆统计通过 `egosync.db` 对应 Service 获取。
- **AR-INC-4**：会话数通过现有对话/Session Service 获取，遵守 `conversations.db` 或 opencode Session 数据边界。
- **AR-INC-5**：角色目标沿用 Role Command → Service → DB 数据链路。
- **AR-INC-6**：前端继续使用 React、TypeScript 和 Tailwind utility，不新增独立 CSS 范式。
- **AR-INC-7**：默认语言由应用初始化/打包配置确定，并优先读取已有用户语言偏好。

### Incremental UX Design Requirements

- **UX-DR-INC-1**：`@Skill` 选择器依附当前管家或角色上下文，仅展示已启用 Skill，并处理空列表与不可用状态。
- **UX-DR-INC-2**：仪表盘展示任务总数、未完成任务数、记忆总数和会话数。
- **UX-DR-INC-3**：仪表盘明确展示当前筛选范围，支持“全部角色”和单角色切换。
- **UX-DR-INC-4**：管家与角色设置页采用一致的可折叠分组交互。
- **UX-DR-INC-5**：“全部折叠”和“全部展开”作用于当前页面全部设置分组。
- **UX-DR-INC-6**：MCP Server 开关明确表达当前启用状态。
- **UX-DR-INC-7**：“自定义 Skill”只位于“Skill 配置”分组。
- **UX-DR-INC-8**：“保存角色信息”位于角色信息分组内，样式对齐“保存使命宣言”。
- **UX-DR-INC-9**：任务时间选择器仅在日期、小时和分钟完整后自动关闭。
- **UX-DR-INC-10**：Markdown 表格支持宽内容防溢出并适配浅色和深色主题。
- **UX-DR-INC-11**：建议对话框及右上角导航标签覆盖默认、悬停、选中和禁用的深色状态。
- **UX-DR-INC-12**：首次启动中文文案与主要导航保持一致，不出现语言混杂。
## Incremental FR Coverage Map: FR-37–FR-46

| Functional Requirement | Epic |
|---|---|
| FR-37 | Epic 9：管家与角色能力配置体验 |
| FR-38 | Epic 10：管家全局洞察 |
| FR-39 | Epic 9：管家与角色能力配置体验 |
| FR-40 | Epic 9：管家与角色能力配置体验 |
| FR-41 | Epic 11：日常交互与内容呈现完善 |
| FR-42 | Epic 9：管家与角色能力配置体验 |
| FR-43 | Epic 11：日常交互与内容呈现完善 |
| FR-44 | Epic 12：一致的产品体验与首次启动 |
| FR-45 | Epic 12：一致的产品体验与首次启动 |
| FR-46 | Epic 12：一致的产品体验与首次启动 |

## Incremental Epic List: FR-37–FR-46

### Epic 9：管家与角色能力配置体验

用户可以清晰配置管家和角色的能力，只调用已启用的 Skill，控制 MCP Server 的启停，并完整维护角色信息。

**FRs covered:** FR-37、FR-39、FR-40、FR-42

### Epic 10：管家全局洞察

用户可以在管家仪表盘快速了解任务、记忆和会话规模，并按角色查看对应统计数据。

**FRs covered:** FR-38

### Epic 11：日常交互与内容呈现完善

用户创建定时任务时操作更流畅，并能在对话及富文本内容中正确查看 Markdown 表格。

**FRs covered:** FR-41、FR-43

### Epic 12：一致的产品体验与首次启动

用户在产品命名、深色模式和首次启动语言方面获得一致且符合预期的体验。
...
### Story 9.1: 通过 @ 显式调用已启用的 Skill

As a 管家或角色的使用者,
I want 在输入消息时通过 EgoSync 的 `@Skill` 选择器选择并显式调用当前上下文已启用的 Skill,
So that 我可以明确控制本次对话使用哪项能力，同时避免调用未经启用的 Skill。

**Acceptance Criteria:**

**Given** 用户正在管家或角色的对话输入框中输入消息
**When** 用户输入 `@`
**Then** 系统显示当前管家或角色已启用的 Skill 候选列表
**And** 不显示已关闭、未分配或不可用的 Skill
**And** 候选项能够清晰展示 Skill 名称及用于辨识该 Skill 的必要信息

**Given** 当前上下文存在多个已启用的 Skill
**When** 用户继续输入 `@` 后的字符
**Then** 候选列表按照输入内容进行匹配过滤
**And** 用户可以通过鼠标或键盘选择候选项
**And** 选择结果以明确的 `@Skill` 标记插入输入内容

**Given** 用户已在消息中选择一个可用的 `@Skill`
**When** 用户发送消息
**Then** 系统通过现有 Tauri IPC 和 Agent 配置链路传递选择结果
**And** 后端将选择结果映射到 OpenCode 原生 Skill 加载机制
**And** 本次执行明确使用用户选择的 Skill
**And** 不需要重启应用

**Given** 用户已插入某个 `@Skill`，但该 Skill 在消息发送前被关闭、删除或变为不可用
**When** 用户发送消息
**Then** 系统不得执行该 Skill
**And** 向用户显示明确的不可用反馈
**And** 不得静默降级为调用其他 Skill
**And** 用户原始输入内容不得丢失，以便修改后重新发送

**Given** 当前管家或角色没有任何已启用的 Skill
**When** 用户输入 `@`
**Then** 系统显示没有可用 Skill 的空状态
**And** 不展示其他角色或其他上下文中的 Skill
**And** 用户仍可继续输入并发送不指定 Skill 的普通消息

**Given** 用户输入普通消息且没有选择 `@Skill`
**When** 用户发送消息
**Then** 系统保持现有对话发送行为
**And** 不强制用户选择 Skill

**Given** 用户在管家对话与角色对话之间切换
**When** 用户分别打开 `@Skill` 候选列表
**Then** 每个对话只展示其当前上下文已启用的 Skill
**And** 上一个上下文的候选状态不得泄漏到新上下文

**Given** Skill 启用状态发生变化
**When** 用户再次打开 `@Skill` 候选列表
**Then** 候选列表反映最新状态
**And** 前端不得直接读取或修改 `opencode.json`

### Story 9.2: 折叠和展开管家设置分组

As a 管家设置页面的使用者,
I want 独立折叠或展开设置分组，并能一次折叠或展开全部分组,
So that 我可以减少页面信息干扰，并快速定位需要修改的设置。

**Acceptance Criteria:**

**Given** 用户进入管家设置页面
**When** 页面完成加载
**Then** 每个设置分组都有明确的标题和折叠/展开控制
**And** 用户能够辨识每个分组当前是折叠还是展开状态
**And** 原有设置项仍位于其对应分组中

**Given** 某个设置分组当前处于展开状态
**When** 用户点击该分组的折叠控制
**Then** 该分组的设置内容被隐藏
**And** 其他设置分组的展开状态保持不变
**And** 分组标题及展开入口仍然可见

**Given** 某个设置分组当前处于折叠状态
**When** 用户点击该分组的展开控制
**Then** 该分组的设置内容恢复显示
**And** 其他设置分组的展开状态保持不变

**Given** 页面存在一个或多个展开的设置分组
**When** 用户点击“全部折叠”
**Then** 当前管家设置页面的全部设置分组均变为折叠状态
**And** “全部折叠”不得影响其他页面的分组状态

**Given** 页面存在一个或多个折叠的设置分组
**When** 用户点击“全部展开”
**Then** 当前管家设置页面的全部设置分组均变为展开状态
**And** 每个分组中的设置内容恢复显示

**Given** 用户已经修改某个设置项但尚未保存
**When** 用户折叠该分组、展开其他分组或执行全部折叠/展开
**Then** 尚未保存的输入值和选择状态保持不变
**And** 折叠操作本身不得触发保存或丢弃更改

**Given** 用户通过键盘操作页面
**When** 焦点位于分组标题或折叠/展开按钮
**Then** 用户可以通过键盘切换分组状态
**And** 控件具有可识别的展开状态及无障碍名称

**Given** 应用处于浅色或深色模式
**When** 用户查看或操作分组控制按钮
**Then** 标题、边界、图标及交互状态保持清晰可辨
**And** 使用项目现有 React、TypeScript 和 Tailwind 样式规范，不引入独立样式范式

### Story 9.3: 与角色设置一致的管家 MCP Server 配置

As a 管家配置的使用者,
I want 以与角色设置页面一致的方式查看和启用或关闭管家的 MCP Server,
So that 我无需学习两套不同的 MCP Server 配置方式，并能明确控制管家当前可使用的外部工具能力。

**Acceptance Criteria:**

**Given** 用户进入管家设置页面的“MCP Server配置”分组
**When** 页面完成加载
**Then** 系统以与角色设置页面一致的列表、状态和操作方式展示管家的 MCP Server
**And** 分组标题、字段名称、开关或操作控件的语义与角色设置页面保持一致
**And** 当前管家的 MCP Server 状态与后端实际配置一致

**Given** 管家 MCP Server 配置页面存在已启用和已关闭的 MCP Server
**When** 用户查看列表
**Then** 每个 MCP Server 都明确展示当前启用状态
**And** 启用、关闭、加载中、保存成功和失败状态的表达方式与角色设置页面一致
**And** 不使用“外部 MCP 工具”作为该分组的用户可见名称

**Given** 某个 MCP Server 当前处于关闭状态
**When** 用户按照角色设置页面相同的交互方式将其启用
**Then** 系统通过 Tauri IPC 提交启用请求
**And** 后端通过现有 `agent_config` 链路更新配置并同步至 OpenCode
**And** 页面按照角色设置页面的成功反馈方式显示已启用状态
**And** 不需要重启应用

**Given** 某个 MCP Server 当前处于启用状态
**When** 用户按照角色设置页面相同的交互方式将其关闭
**Then** 系统通过 Tauri IPC 提交关闭请求
**And** 后端通过现有 `agent_config` 链路更新配置并同步至 OpenCode
**And** 页面按照角色设置页面的成功反馈方式显示已关闭状态
**And** 后续管家请求不得调用该 MCP Server

**Given** 用户关闭了某个 MCP Server
**When** 该 MCP Server 仍存在于管家的可配置能力列表中
**Then** 系统不得继续向 OpenCode 发起该 MCP Server 的工具调用
**And** 页面明确展示其关闭状态或不可用反馈
**And** 系统不得静默启用或切换其他 MCP Server

**Given** MCP Server 状态更新失败
**When** 系统收到失败结果
**Then** 开关或状态恢复为更新前的状态
**And** 使用与角色设置页面一致的错误反馈方式提示用户
**And** 不得显示与实际配置不一致的成功状态

**Given** 用户快速连续切换同一个 MCP Server 的状态
**When** 前一个更新请求尚未完成
**Then** 系统使用与角色设置页面一致的加载或禁用交互，避免并发操作造成状态错乱
**And** 最终页面状态与后端最终配置一致

**Given** 用户离开管家设置页面后重新进入
**When** 页面重新加载 MCP Server 配置
**Then** 页面显示最新状态
**And** 状态不会因为页面刷新或重新进入而恢复为旧值

**Given** 应用处于浅色或深色模式
**When** 用户查看或操作管家 MCP Server 配置
**Then** 所有状态和交互样式与角色设置页面保持一致
**And** 启用、关闭、悬停、聚焦、加载中和失败状态均清晰可辨

**Given** 前端需要读取或更新 MCP Server 配置
**When** 用户执行对应操作
**Then** 前端通过现有 Tauri IPC 调用后端
**And** 前端不得直接读取或修改 `opencode.json`

### Story 9.4: 整理并折叠角色设置分组

As a 角色设置页面的使用者,
I want 通过清晰一致的分组管理角色设置,
So that 我可以快速找到 Skill、MCP Server 和其他角色配置，并减少页面滚动和信息干扰。
...
**When** 页面完成加载
**Then** 每个设置分组都有明确的标题和折叠/展开控制
**And** 分组内的设置项按照当前产品信息架构归类
**And** 用户能够辨识每个分组当前是折叠还是展开状态

**Given** 用户操作单个分组的折叠/展开控制
**When** 分组状态发生切换
**Then** 仅切换当前分组的内容显示状态
**And** 其他分组状态保持不变

**Given** 页面存在多个角色设置分组
**When** 用户点击“全部折叠”或“全部展开”
**Then** 当前角色设置页面的全部分组切换到对应状态
**And** 不影响管家设置页面或其他页面的分组状态

**Given** 用户查看 Skill 相关设置
**When** 页面完成分组渲染
**Then** “自定义 Skill”只出现在“Skill 配置”分组中
**And** 不在其他分组重复展示
**And** 现有 Skill 设置仍可正常使用

**Given** 用户查看 MCP 相关设置
**When** 页面完成分组渲染
**Then** 相关分组和操作统一使用“MCP Server配置”或项目确定的等价中文文案
**And** 不再使用“外部 MCP 工具”作为用户可见名称
**And** MCP Server 的既有功能保持可用

**Given** 用户已经修改某个角色设置项但尚未保存
**When** 用户切换分组状态或执行全部折叠/展开
**Then** 尚未保存的输入值和选择状态保持不变
**And** 折叠操作不得触发保存或丢弃更改

**Given** 角色设置页面完成本次分组调整
**When** 用户查看页面底部
**Then** 不再显示全局“保存更改”按钮
**And** 角色信息的保存入口由角色信息分组内的专用按钮提供
...
**Given** 应用处于浅色或深色模式
**When** 用户查看或操作角色设置分组
**Then** 标题、边界、图标和交互状态均清晰可辨
**And** 角色设置页面与管家设置页面使用一致的折叠/展开交互风格

### Story 9.5: 在角色信息分组内独立保存角色信息

As a 角色设置页面的使用者,
I want 在角色信息分组内单独保存角色的基本信息,
So that 我可以明确知道保存操作影响哪些内容，并避免其他设置分组受到影响。

**Acceptance Criteria:**

**Given** 用户进入角色设置页面并展开“角色信息”分组
**When** 页面完成加载
**Then** 分组内显示“保存角色信息”按钮
**And** 按钮的位置和视觉样式与管家设置页面的“保存使命宣言”按钮保持一致
**And** 页面底部不再显示全局“保存更改”按钮

**Given** 用户修改角色信息分组中的字段
**When** 用户点击“保存角色信息”
**Then** 系统仅提交角色信息分组中的字段
**And** 通过现有 Tauri IPC 调用 Role Command → Service → Database 链路保存
**And** 不提交或覆盖 Skill、MCP Server及其他设置分组中的数据

**Given** 用户在其他设置分组中存在尚未保存的修改
**When** 用户点击“保存角色信息”
**Then** 其他分组中的未保存状态和输入值保持不变
**And** 系统不得将其他分组的修改标记为已保存

**Given** 用户点击“保存角色信息”
**When** 保存请求正在执行
**Then** 按钮显示明确的处理中状态
**And** 系统避免重复提交相同请求
**And** 用户仍能识别当前正在保存的是角色信息

**Given** 角色信息保存成功
**When** 后端返回更新后的角色数据
**Then** 页面使用后端返回结果更新角色信息
**And** 显示明确的保存成功反馈
**And** 角色列表、角色标题或其他展示该信息的位置同步反映最新数据

**Given** 角色信息保存失败
**When** 后端返回错误或请求无法完成
**Then** 页面显示可理解的失败原因或重试提示
**And** 用户本次输入内容保持不变
**And** Skill、MCP Server及其他设置分组的状态不受影响
**And** 页面不得将未保存数据展示为已经保存

**Given** 用户没有修改角色信息
**When** 用户查看“保存角色信息”按钮
**Then** 按钮处于禁用状态或采用项目现有的等价防重复提交处理
**And** 不发送无变化的保存请求

**Given** 用户修改了角色信息但尚未保存
**When** 用户折叠并重新展开角色信息分组
**Then** 未保存的输入内容保持不变
**And** 折叠操作不得自动保存或丢弃输入内容

**Given** 应用处于浅色或深色模式
**When** 用户查看或操作“保存角色信息”按钮
**Then** 默认、悬停、聚焦、禁用、处理中、成功和失败状态均清晰可辨
**And** 按钮样式遵循项目现有 React、TypeScript 和 Tailwind 规范

### Story 9.6: 创建和编辑角色目标

As a 创建或维护角色的使用者,
I want 在添加和编辑角色时填写、查看并保存角色目标,
So that 每个角色都能拥有明确且可维护的行动方向。

**Acceptance Criteria:**

**Given** 用户打开“添加角色”表单
**When** 表单完成加载
**Then** 角色信息区域显示清晰可见的“目标”输入项
**And** 目标字段与名称、职责等角色核心信息一起展示
**And** 用户可以在提交前填写和修改目标

**Given** 用户在添加角色时填写了目标
**When** 用户提交角色创建表单
**Then** 系统通过现有 Tauri IPC 和 Role Command → Service → Database 链路保存目标
**And** 创建成功后返回的角色数据包含该目标
**And** 不需要通过创建后的二次编辑补充目标

**Given** 用户打开已有角色的设置页面
**When** 用户展开“角色信息”分组
**Then** 页面显示该角色当前保存的目标
**And** 用户可以修改目标
**And** 目标与其他角色信息使用角色信息分组内的“保存角色信息”按钮保存

**Given** 用户修改已有角色的目标
**When** 用户点击“保存角色信息”且请求成功
**Then** 更新后的目标被持久化
**And** 重新进入角色设置页面后仍显示最新目标
**And** 角色详情或其他展示目标的位置同步反映最新内容

**Given** 用户修改角色目标后保存失败
**When** 后端返回错误
**Then** 页面保留用户输入的目标内容
**And** 显示明确的失败反馈或重试提示
**And** 不得把未保存的目标展示为已保存状态
**And** 其他设置分组不受影响

**Given** 用户通过对话流程创建角色
**When** 系统从对话内容中提炼出角色目标
**Then** 提炼结果被回填到创建确认界面的“目标”字段
**And** 用户能够在创建角色前查看、修改或清空提炼结果
**And** 系统不得绕过用户确认直接保存提炼目标

**Given** 用户已经手动修改对话创建流程中的目标字段
**When** 较晚返回的提炼结果到达
**Then** 系统不得静默覆盖用户已经修改的内容
**And** 用户可以明确选择是否采用新的提炼结果

**Given** 角色创建或编辑流程对目标字段存在长度或格式限制
**When** 用户输入不符合限制的内容
**Then** 页面显示明确的验证信息
**And** 创建与编辑流程采用一致的验证规则
**And** 用户已经输入的其他角色信息保持不变
...
### Story 10.1: 展示全部角色的管家仪表盘统计

As a 使用管家仪表盘的用户,
I want 查看全部活跃角色的任务、记忆和会话统计,
So that 我可以快速了解当前整体工作量和知识积累情况。

**Acceptance Criteria:**

**Given** 用户进入管家仪表盘
**When** 统计数据加载成功
**Then** 页面展示任务总数、未完成任务数、记忆总数和会话数
**And** 每项统计都有清晰且不产生歧义的中文标签
**And** 页面明确显示当前统计范围为“全部角色”

**Given** 当前统计范围为“全部角色”
**When** 系统计算四项统计值
**Then** 统计范围包括全部活跃角色
**And** 不包含已归档、已删除或不属于当前用户的数据
**And** 四项统计采用相同的角色范围

**Given** 系统计算任务统计
**When** 请求任务总数和未完成任务数
**Then** 数据通过现有 Task Service 从 `egosync.db` 获取
**And** 任务总数包含当前范围内的全部有效任务
**And** 未完成任务数按照系统现有任务状态规则排除已完成任务
**And** 未完成任务数不得大于任务总数

**Given** 系统计算记忆总数
**When** 请求当前范围的记忆统计
**Then** 数据通过现有 Memory Service 从 `egosync.db` 获取
**And** 只统计当前全部活跃角色范围内的有效记忆

**Given** 系统计算会话数
**When** 请求当前范围的会话统计
**Then** 数据通过现有 Conversation/Session Service 获取
**And** 遵守 `conversations.db` 或 OpenCode Session 的现有数据边界
**And** 不通过任务或消息数量推算会话数

**Given** 前端加载仪表盘统计
**When** 页面向后端请求数据
**Then** 前端通过 Tauri IPC 调用统计接口
**And** 前端不得直接访问数据库或 OpenCode Server
**And** 四项统计结果明确对应同一次“全部角色”统计范围

**Given** 当前范围没有任何任务、记忆或会话
**When** 统计请求成功
**Then** 对应统计值显示为 `0`
**And** 页面不显示空白、`null`、`undefined` 或误导性的加载状态
**And** 零值状态不应被展示为查询失败

**Given** 统计请求正在执行
**When** 用户查看仪表盘
**Then** 四项统计显示一致的加载状态
**And** 不短暂显示可能被误认为真实结果的错误零值

**Given** 一个或多个统计来源查询失败
**When** 页面无法获得完整统计结果
**Then** 页面显示明确的失败状态和可重试入口
**And** 不把查询失败显示为 `0`
**And** 已成功获得的数据不得被错误标记为失败

**Given** 统计数据发生变化
**When** 用户重新进入仪表盘或触发现有刷新机制
**Then** 页面显示最新统计结果
**And** 四项统计继续使用一致的数据范围

**Given** 应用处于浅色或深色模式
**When** 用户查看统计区域
**Then** 标签、数字、图标、边界、加载和错误状态均清晰可读
**And** 统计区域遵循现有仪表盘视觉规范

### Story 10.2: 按角色筛选仪表盘统计与内容

As a 使用管家仪表盘的用户,
I want 选择一个角色查看其任务、记忆和会话统计,
So that 我可以快速了解特定角色的工作状态和知识积累情况。

**Acceptance Criteria:**

**Given** 用户进入管家仪表盘
**When** 查看角色筛选控件
**Then** 筛选选项包括“全部角色”和当前全部活跃角色
**And** 不显示已归档、已删除或不属于当前用户的角色
**And** 当前选中的统计范围清晰可见

**Given** 当前筛选范围为“全部角色”
**When** 用户选择一个活跃角色
**Then** 系统重新加载任务总数、未完成任务数、记忆总数和会话数
**And** 四项统计均只计算所选角色的数据
**And** 不出现部分统计使用全部角色、部分统计使用单角色的混合状态

**Given** 用户选择一个角色
**When** 筛选结果加载成功
**Then** 仪表盘明确展示所选角色的名称或等价范围标识
**And** 角色状态卡片及与统计关联的任务、记忆或会话列表同步使用该角色范围
**And** 不属于所选角色的数据不再出现在筛选结果中

**Given** 当前已经选择某个角色
**When** 用户选择“全部角色”或清除筛选
**Then** 四项统计恢复为全部活跃角色汇总
**And** 角色状态卡片及相关列表恢复全部角色范围
**And** 页面明确显示当前范围为“全部角色”

**Given** 所选角色没有任务、记忆或会话数据
**When** 筛选请求成功
**Then** 对应统计值显示为 `0`
**And** 相关区域显示与当前角色对应的空状态
**And** 不错误地回退显示其他角色的数据

**Given** 用户快速连续切换角色
**When** 多个统计请求先后返回
**Then** 页面最终只显示当前所选角色的数据
**And** 较早请求的响应不得覆盖当前筛选结果
**And** 加载状态与当前所选角色保持一致
...
**Then** 页面保留当前筛选范围标识
**And** 显示明确的失败状态和重试入口
**And** 不将上一角色的统计数据错误显示为当前角色数据
**And** 不把查询失败显示为零值

**Given** 当前选中的角色被归档、删除或变为不可用
**When** 仪表盘重新加载筛选数据
**Then** 系统安全恢复到“全部角色”范围
**And** 向用户说明原筛选角色已不可用
**And** 四项统计及相关内容使用恢复后的统一范围

**Given** 用户离开仪表盘后重新进入
**When** 页面显示统计内容
**Then** 页面明确显示实际使用的筛选范围
**And** 筛选控件、四项统计、角色状态卡片和相关列表的范围保持一致
**And** 不得出现界面显示某角色但实际统计全部角色的情况

**Given** 应用处于浅色或深色模式
**When** 用户查看或操作角色筛选控件
**Then** 默认、悬停、聚焦、展开、选中、加载和禁用状态均清晰可辨
**And** 当前选中角色具有明确的视觉状态
...
### Story 11.1: 完成任务时间选择后自动关闭弹窗

As a 创建任务的用户,
I want 在日期、小时和分钟都选择完成后自动关闭时间弹窗,
So that 我可以减少额外点击并顺畅地继续填写或提交任务。

**Acceptance Criteria:**

**Given** 用户打开创建任务弹窗中的时间选择器
**When** 日期、小时或分钟中的任意一项尚未完成选择
**Then** 时间选择弹窗保持打开
**And** 系统不得因为只选择了部分时间而强制关闭弹窗
**And** 已完成的选择保持不变

**Given** 用户已经选择日期和小时，但尚未选择分钟
**When** 用户选择分钟
**Then** 系统确认日期、小时和分钟均已完整
**And** 自动关闭时间选择弹窗
**And** 将完整时间回填到创建任务表单

**Given** 用户以不同顺序选择日期、小时和分钟
**When** 最后一项必要信息完成选择
**Then** 时间选择弹窗自动关闭
**And** 自动关闭行为不依赖固定的选择顺序
**And** 回填结果使用用户最终选择的完整时间

**Given** 用户完成日期、小时和分钟选择
**When** 时间选择弹窗自动关闭
**Then** 创建任务弹窗保持打开
**And** 用户已经填写的任务标题、描述、角色及其他字段保持不变
**And** 焦点返回创建任务流程中的合理位置

**Given** 时间选择结果已经回填
**When** 用户查看创建任务表单
**Then** 页面以清晰且符合当前语言设置的格式显示所选日期和时间
**And** 表单内部保存的时间值与界面显示一致
**And** 不因自动关闭重复提交或直接创建任务

**Given** 用户需要修改已经选择的时间
**When** 用户重新打开时间选择弹窗
**Then** 弹窗显示此前选择的日期、小时和分钟
**And** 用户可以修改任意部分
**And** 完成新的完整选择后再次自动关闭并回填最新值

**Given** 用户清除日期或时间中的必要部分
**When** 日期、小时或分钟不再完整
**Then** 系统不得使用残缺值覆盖原有完整时间
**And** 弹窗保持打开，直到用户完成选择或主动取消

**Given** 用户主动点击弹窗外部、关闭按钮或取消操作
**When** 当前选择尚未完整
**Then** 弹窗按照现有取消规则关闭
**And** 不将残缺的日期或时间回填到任务表单
**And** 创建任务表单中的其他内容保持不变

**Given** 用户通过键盘操作时间选择器
**When** 用户完成日期、小时和分钟选择
**Then** 自动关闭行为与鼠标操作一致
**And** 弹窗关闭后焦点不会丢失到页面背景中的不可见元素

**Given** 应用处于浅色或深色模式
**When** 用户选择日期、小时和分钟
**Then** 已选、悬停、聚焦、禁用和当前时间状态均清晰可辨
**And** 自动关闭不得引起明显的布局跳动

### Story 11.2: 正确渲染标准 Markdown 表格

As a 查看管家或角色回复的用户,
I want 对话及其他富文本区域中的标准 Markdown 表格被正确渲染,
So that 我可以清晰阅读结构化信息，而不是看到未解析的竖线和分隔符。

**Acceptance Criteria:**

**Given** 对话消息包含符合标准语法的 Markdown 表格
**When** 系统渲染该消息
**Then** 表头、分隔行和数据行被渲染为结构清晰的表格
**And** 不将原始竖线和表格分隔符作为普通文本显示
**And** 表头与数据单元格保持正确的行列关系

**Given** 同一条消息同时包含普通段落、标题、列表、代码块和 Markdown 表格
**When** 系统渲染完整消息
**Then** 每种 Markdown 内容均按照对应语义正确显示
**And** 表格渲染不会破坏其他内容的布局或格式
**And** 内容顺序与原始 Markdown 一致

**Given** Markdown 表格包含中文、数字、空单元格或标点符号
**When** 系统渲染表格
**Then** 各类内容显示在正确单元格中
**And** 空单元格不会造成后续列错位
**And** 中文内容保持正常换行和可读性

**Given** 表格单元格包含系统已支持的行内 Markdown
**When** 系统渲染表格
**Then** 加粗、斜体、行内代码或链接按照现有 Markdown 规则显示
**And** 行内格式不会破坏表格结构

**Given** Markdown 表格的总宽度超过对话消息或富文本容器宽度
**When** 用户查看该表格
**Then** 表格区域提供横向滚动或项目确定的等价防溢出处理
**And** 表格不得撑破消息气泡、页面主体或侧边栏布局
**And** 页面整体不因单个宽表格产生不必要的横向滚动

**Given** 表格中包含较长且无法自然换行的内容
**When** 内容超过单元格可用宽度
**Then** 内容按照统一规则换行、截断或在表格容器内滚动
**And** 不与相邻单元格内容重叠
**And** 用户仍能访问完整内容

**Given** 输入文本类似表格但不符合标准 Markdown 表格语法
**When** 系统尝试渲染该消息
**Then** 整条消息仍可正常显示
**And** 非法表格内容不会导致渲染器报错、白屏或隐藏其他内容
**And** 系统按照现有 Markdown 降级规则展示该部分文本

**Given** Markdown 内容来源于流式生成的对话回复
**When** 表格语法尚未完整或后续内容继续到达
**Then** 消息区域保持稳定且不会导致应用报错
**And** 当完整表格内容到达后，系统能够显示最终的正确表格
**And** 不重复显示同一份表格内容

**Given** 应用处于浅色或深色模式
**When** 用户查看 Markdown 表格
**Then** 表头、单元格、边框、背景和悬停状态清晰可读
**And** 表格样式与当前主题及现有对话界面保持一致

**Given** 应用在对话消息以外的富文本区域复用同一 Markdown 渲染能力
**When** 内容包含标准 Markdown 表格
**Then** 表格采用一致的解析和展示规则
**And** 不为不同页面实现互相冲突的表格语法或样式

**Given** Markdown 内容包含不受信任的 HTML 或脚本
**When** 系统渲染包含表格的内容
**Then** 继续遵守现有 Markdown 安全和内容清理规则
**And** 表格支持不得绕过现有安全限制
...
### Story 12.1: 统一产品用户可见名称

As a EgoSync 用户,
I want 产品中统一使用“数字分身管家”这一名称,
So that 我可以在不同页面和流程中获得一致的产品认知。

**Acceptance Criteria:**

**Given** 用户查看应用中的管家相关页面
**When** 页面渲染标题、导航、按钮或说明文案
**Then** 用户可见的产品名称统一使用“数字分身管家”
**And** 不再以独立旧文案“分身管家”作为产品名称展示
**And** 文案含义与当前页面上下文保持一致

**Given** 用户查看管家仪表盘、管家设置和角色相关入口
**When** 系统展示页面标题、导航标签、菜单、操作按钮或空状态
**Then** 所有相关用户可见名称均采用统一的新名称
**And** 不出现同一产品在不同页面使用不同名称的情况

**Given** 用户查看首次引导、帮助提示、确认弹窗或通知
**When** 内容引用管家产品名称
**Then** 文案使用“数字分身管家”
**And** 不遗漏引导和异常状态等非主页面文案

**Given** 应用支持多语言文案
**When** 当前语言为中文
**Then** 中文翻译资源中的管家产品名称统一更新为“数字分身管家”
**And** 不因部分页面仍使用旧翻译键而显示旧名称
**And** 其他语言的既有翻译结构保持可用

**Given** 用户可见名称发生更新
**When** 应用加载已有管家、角色或历史数据
**Then** 已保存的管家、角色名称和用户自定义内容不会被无故修改
**And** 本次调整只影响产品固定文案，不覆盖用户输入的数据

**Given** 内部代码、数据库字段、接口名称或迁移脚本中存在历史标识
**When** 应用正常运行
**Then** 不要求仅因文案调整而修改稳定的内部标识
**And** 用户界面不得将内部历史标识直接展示给用户

**Given** “数字分身管家”出现在按钮、卡片、标题或窄宽度容器中
**When** 页面完成布局
**Then** 文案不遮挡其他控件
**And** 不产生异常截断、重叠或布局溢出
**And** 浅色和深色模式下均保持可读

**Given** 用户通过搜索、帮助或页面文本查找管家相关功能
**When** 系统展示匹配结果
**Then** 固定产品文案使用“数字分身管家”
**And** 不因残留旧固定文案造成同一功能出现两个名称

### Story 12.2: 补齐建议对话框和导航标签的深色模式

As a 使用深色模式的 EgoSync 用户,
I want 建议对话框和右上角导航标签正确适配深色模式,
So that 我可以在深色环境下清晰阅读并准确识别当前页面和可用操作。

**Acceptance Criteria:**

**Given** 应用切换到深色模式
**When** 用户打开建议对话框
**Then** 对话框背景、边框、标题、正文、按钮、输入区域和关闭控件均使用深色主题样式
**And** 不出现默认浅色背景或浅色主题文字
**And** 对话框与现有深色页面的层级和视觉风格保持一致

**Given** 建议对话框处于深色模式
**When** 用户查看建议内容或执行操作
**Then** 建议文本、辅助说明和可操作控件清晰可读
**And** 默认、悬停、聚焦、按下、禁用和错误状态均具有明确区别
**And** 控件状态不会因背景颜色相近而难以识别

**Given** 用户在深色模式下查看页面右上角导航标签
**When** 导航展示“仪表盘”“任务”等页面选项
**Then** 未选中标签、选中标签和不可用标签均采用深色主题对应样式
**And** 当前选中标签具有清晰且稳定的视觉标识
**And** 选中状态不再沿用仅适用于浅色模式的背景、文字或边框颜色

**Given** 用户在深色模式下将鼠标移动到右上角导航标签
**When** 标签进入悬停状态
**Then** 悬停样式与默认和选中样式保持可区分
**And** 悬停状态不会降低标签文字的可读性

**Given** 用户通过键盘操作建议对话框或右上角导航标签
**When** 控件获得焦点
**Then** 深色模式下焦点指示清晰可见
**And** 焦点状态不会被选中背景或对话框背景遮挡
**And** 交互顺序和现有页面保持一致

**Given** 用户在浅色模式和深色模式之间切换
**When** 当前页面包含建议对话框或右上角导航标签
**Then** 两类组件立即使用对应主题样式
**And** 当前选中的导航项、打开的对话框及用户输入内容保持不变
**And** 切换主题不得触发页面跳转或丢失未提交内容

**Given** 用户在不同页面之间切换
**When** 右上角导航标签的选中项发生变化
**Then** 新选中标签在深色模式下继续使用正确的选中样式
**And** 原选中标签恢复为正确的未选中样式
**And** 不出现多个标签同时显示为选中状态

**Given** 应用处于深色模式
**When** 用户查看建议对话框或右上角导航标签
**Then** 组件使用项目现有 React、TypeScript 和 Tailwind 主题规范
**And** 不新增与现有主题系统冲突的独立 CSS 方案

### Story 12.3: 新安装首次启动默认使用中文

As a 首次安装 EgoSync 的用户,
I want 应用首次启动时默认使用中文,
So that 我可以直接使用中文完成引导和主要操作，而无需先修改语言设置。

**Acceptance Criteria:**

**Given** 用户全新安装 EgoSync
**And** 本地不存在有效的语言偏好
**When** 用户首次启动应用
**Then** 应用默认使用中文
**And** 首次引导、主要导航、按钮和基础提示均显示中文
**And** 默认语言不依赖操作系统当前语言

**Given** 用户在 Windows、macOS 或 Linux 上进行全新安装
**When** 应用首次启动且不存在语言偏好
**Then** 三个平台均默认使用中文
**And** 打包配置与应用初始化逻辑采用一致的中文默认值

**Given** 应用正在读取首次启动配置
**When** 中文界面完成初始化
**Then** 页面不应先显示其他语言再闪烁切换为中文
**And** 首次可见界面不出现中文与其他语言混杂
**And** 首次引导和主要导航使用相同语言

**Given** 用户之前已经选择某种语言
**When** 用户升级应用或再次启动
**Then** 系统优先使用已保存的语言偏好
**And** 不因新的中文默认配置覆盖已有选择
**And** 选择非中文的既有用户升级后继续使用原语言
...
**Given** 本地语言偏好缺失、为空或无法识别
**When** 应用无法得到有效的既有语言值
**Then** 系统安全回退到中文
**And** 应用能够正常完成启动
**And** 不显示空白语言、翻译键或初始化错误页面

**Given** 本地存在受支持的有效语言偏好
**When** 应用启动
**Then** 该语言偏好优先于打包默认值和中文回退值
**And** 系统不得仅依据操作系统语言覆盖用户选择

**Given** 新安装用户进入首次引导并随后进入主应用
**When** 页面在不同启动阶段之间切换
**Then** 首次引导、管家页面、角色页面和主要导航均保持中文
**And** 不因不同模块使用不同默认逻辑而发生语言切换

**Given** 应用读取语言资源失败
**When** 当前首选语言资源暂时不可用
**Then** 系统按照现有错误处理机制安全回退到中文基础资源
**And** 不因语言资源异常导致应用无法启动
**And** 必要的错误信息应保持可理解

**Given** 开发者构建发布安装包
**When** 执行生产构建
**Then** 中文默认值包含在正式打包配置或应用初始化配置中
**And** 该默认值不只存在于开发环境或测试 Mock 中
**And** 自动化测试覆盖全新用户与已有语言偏好用户两种启动路径
