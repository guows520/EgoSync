//! dashboard 域命令体（Story 15.4 自壳 `commands/dashboard.rs` 平移，
...
pub async fn dashboard_get_status(ctx: &EngineCtx) -> Result<Vec<DashboardStatus>, AppError> {
    dashboard_service::get_dashboard_status(&ctx.pool, &ctx.conv_pool).await
}
...
pub async fn dashboard_get_metrics(
    ...
    dashboard_service::get_dashboard_metrics(&ctx.pool, &ctx.conv_pool, query).await
}
