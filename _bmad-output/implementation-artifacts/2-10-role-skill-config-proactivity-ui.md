...
- [x] 后端：补齐角色 Skill 与主动性更新 API（AC: 2, 3, 6）
  - [x] 在 `egosync-app/src-tauri/src/models/role.rs` 增加专用输入 DTO：Skill 配置更新与主动性更新；保持 `#[serde(rename_all = "camelCase")]`。
  - [x] 在 `egosync-app/src-tauri/src/db/roles.rs` 增加只更新 `skills_config` 与只更新 `proactivity_level` 的 helper，返回完整 `Role`。
  - [x] 在 `egosync-app/src-tauri/src/commands/role.rs` 增加 Tauri command：`role_update_skills` 与 `role_update_proactivity`。
  - [x] 在 `egosync-app/src-tauri/src/lib.rs` 的 `tauri::generate_handler!` 注册新增 command。
...
- [x] 后端：定义并验证 Skill JSON、元 Skill 生效映射与关闭阻断（AC: 2, 3, 4）
  ...
  - [x] 确保 `egosync-app/src-tauri/src/services/agent_config.rs` 能把角色启用的元 Skill 写入 opencode agent 提示约束；禁用的元 Skill 不应在角色 prompt 中被描述为可用。
...
- [x] 前端：补齐类型与 service（AC: 1, 2, 5, 6）
  - [x] 在 `egosync-app/src/types/role.ts` 增加 Skill 配置与主动性更新输入类型。
  - [x] 在 `egosync-app/src/services/roleService.ts` 增加 `updateSkills(id, input)` 与 `updateProactivity(id, input)`，通过 Tauri `invoke()` 调用新增 command。
  - [x] 在 `egosync-app/src/services/appService.ts` 增加 `getButlerSkills()` 与 `updateButlerSkills(input)`，通过 Tauri `invoke()` 调用管家 Skill command。
  - [x] 保持组件不直接调用 `invoke()`；所有 IPC 仍走 service 层。

- [x] 前端：替换 SettingsTab 的 mock Skill 区块并补齐管家 Skill 配置（AC: 1, 2, 3, 8)
  - [x] 在 `egosync-app/src/components/role/SettingsTab.tsx` 解析 `role.skillsConfig`，展示两个真实元 Skill 开关：`find-skills`、`skill-creator`。
  - [x] 删除现有硬编码 API key 风格输入 `sk-xxxx-xxxx-xxxx-xxxx` 与 mock Skill 凭据 UI。
  - [x] 在 `egosync-app/src/components/butler/ButlerSettingsContent.tsx` 增加同名 “Skill 配置” 区块，读写管家 Skill 配置。
...
- [x] 前端：改造 ProactivityToggle 为真实受控组件（AC: 5, 6, 7, 8）
  - [x] 将 `egosync-app/src/components/role/ProactivityToggle.tsx` 从本地 mock state 改为受控 props：当前 level、变更回调、可选 pending/disabled 状态。
...
- [x] 对话关闭 Skill 阻断路径（AC: 4）
  - [x] 先检查 `egosync-app/src-tauri/src/services/agent_engine.rs` 现有 opencode event/tool 处理，不发明前端无法接收的新事件协议。
...
- [x] 测试与验证（AC: 1-8）
  ...
  - [x] 更新 `egosync-app/src/components/role/SettingsTab.test.tsx`：mock `roleService.updateSkills` / `updateProactivity`，覆盖两个元 Skill 开关与主动性切换持久化。
  - [x] 更新/新增 `ProactivityToggle` 测试，验证受控渲染与 onChange。
  - [x] 回归现有 SettingsTab 测试：保存角色基础信息、删除确认、仅剩一个 active role 时危险操作禁用。
  - [x] 至少运行：`cd GUI && npm run test:frontend`、`cd egosync-app/src-tauri && cargo test`、`cd GUI && npm run build`。
...
- Rust DB SQL 保持在 `egosync-app/src-tauri/src/db/`。
- Rust command 返回 `Result<T, AppError>`，不要在 command path 使用 `.unwrap()`。
- opencode agent 配置同步由 `egosync-app/src-tauri/src/services/agent_config.rs` 负责；角色更新后使用既有 best-effort sync，不因 opencode 配置写入失败阻塞角色 CRUD。
...
- `egosync-app/src/components/role/ProactivityToggle.tsx`
...
- Frontend: Vitest + React Testing Library under `egosync-app/src/**/*.test.tsx`.
- Rust: `cargo test` under `egosync-app/src-tauri`.
...
- Existing role UI is under `egosync-app/src/components/role/`; keep new UI there.
- Existing role IPC service is `egosync-app/src/services/roleService.ts`; extend it rather than creating a parallel service.
- Existing role persistence is under `egosync-app/src-tauri/src/db/roles.rs`; extend it rather than embedding SQL in commands.
- No `egosync-app/src/hooks/useRoles.ts` exists; do not reference or create it unless a separate refactor is explicitly requested.
...
- Existing schema: `egosync-app/src-tauri/migrations/003_roles.sql` → `skills_config`, `proactivity_level`.
- Existing role model: `egosync-app/src-tauri/src/models/role.rs`.
- Existing role DB updates: `egosync-app/src-tauri/src/db/roles.rs`.
- Existing role commands: `egosync-app/src-tauri/src/commands/role.rs`.
- Existing opencode config mapping: `egosync-app/src-tauri/src/services/agent_config.rs`.
- Existing settings UI: `egosync-app/src/components/role/SettingsTab.tsx`.
- Existing proactivity mock: `egosync-app/src/components/role/ProactivityToggle.tsx`.
...
- 初次在项目根目录执行 Rust 测试时未找到 `Cargo.toml`，已改用 `cargo test --manifest-path "egosync-app/src-tauri/Cargo.toml"`。
- 全局 Cargo registry 指向失效 USTC 镜像，已使用项目本地 `egosync-app/.cargo-home-local` 执行 Rust 验证。
...
- 已运行验证：`npm --prefix "GUI" run test:frontend`（14 个测试文件、103 个测试通过）、`cargo test --manifest-path "egosync-app/src-tauri/Cargo.toml" -- --test-threads=1`（241 个 lib 测试 + 1 个 integration 测试通过）、`npm --prefix "GUI" run build`（TypeScript + Vite 构建通过）。后续回归补充：`cargo test --manifest-path "egosync-app/src-tauri/Cargo.toml" agent_config`（21 passed）、`cargo test --manifest-path "egosync-app/src-tauri/Cargo.toml" meta_skill`（17 passed）、完整 `cargo test --manifest-path "egosync-app/src-tauri/Cargo.toml"`（254 个 lib 测试 + 1 个 integration 测试通过）、`cargo check --manifest-path "egosync-app/src-tauri/Cargo.toml"` 通过。
...
- `egosync-app/src-tauri/src/services/butler_config.rs`
- `egosync-app/src-tauri/src/services/agent_config.rs`
...
- `egosync-app/src/components/role/SettingsTab.tsx`
- `egosync-app/src/components/role/SettingsTab.test.tsx`
- `egosync-app/src/components/role/ProactivityToggle.test.tsx`
