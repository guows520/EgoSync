...
//!
//! - CSP（架构 ⑧ 冻结内容 + 16.1 脚本 hash 裁量）：`default-src 'self';
//!   script-src 'self' 'sha256-…'; style-src 'self' 'unsafe-inline';
//!   font-src 'self'; connect-src 'self'`——全响应下发（含
//!   healthz / 401 / 404 / 静态 HTML）。`script-src` 追加 hash-source
//!   放行 index.html 的内联防 FOUC 主题脚本（禁止整体放开
//!   unsafe-inline；hash 与构建产物 byte 对 byte 钉死——前端 security
//!   契约源码扫描测试守门）。16.1 曾放行的两 Google 字体域已随
//!   Story 17.1 字体自托管（fontsource npm 包，woff2 随 dist 分发）
//!   撤除——font-src/style-src 回归 `'self'`，零第三方域（离线/内网
//!   一致性与隐私）。
...
/// CSP 头值（架构 ⑧ 冻结内容 + 16.1 脚本 hash-source 裁量；字体源已随
/// 17.1 自托管撤除——零第三方域）。
///
...
pub const CSP_POLICY: &str = "default-src 'self'; script-src 'self' \
    ...
    'unsafe-inline'; font-src 'self'; connect-src 'self'";
