...
**2026-08-25更新**：新增§4.14 手机伴侣（Android）。确定移动形态为伴侣App：手机经局域网/云中继连接桌面端引擎，一档18项核心FR与二档6项适配FR进入移动端，新增FR-40至FR-43（配对连接、实时状态同步、移动推送、离线降级）；桌面优先的9个FR明确不在手机端提供。相应修订§6 MVP范围与Non-Goals（原"移动端延期V2"非目标被本次决策取代）及Privacy/Cost/Platform守则。
...
**2026-09-25更新**：§4.15 FR-45 验收标准补「移动端专用形态」与「PWA 可安装」两条（结清 Story 16.2 延期项与 UX-C4 欠账——<768px 底部 tab 导航 + 全屏对话 + 44px 触控 + 安全区避让；PWA = manifest + maskable 图标 + 应用外壳离线缓存，业务数据不落浏览器）；§6.2 Non-Goals 登记 PWA Web 推送为显式后置；云端版 V1 范围不变。详见 sprint-change-proposal-2026-09-25.md。
...
EgoSync的移动形态为**伴侣App**：Android手机作为桌面端引擎的远程视图与指令入口，桌面保持唯一事实源（与FR-25一致）。手机端不运行AI引擎、不持有数据主权；桌面离线时降级为只读缓存与速记排队（FR-43）。既有功能按移动适配度分档进入：

| 分档 | 范围 |
|------|------|
| 核心体验 | FR-1, FR-2, FR-3, FR-11, FR-12, FR-14, FR-15, FR-16, FR-18, FR-19, FR-20, FR-21, FR-22, FR-24, FR-29, FR-30, FR-33, FR-38 |
| 适配可用（查看/轻操作） | FR-5, FR-7, FR-8, FR-9, FR-17, FR-23 |
| 桌面优先（不在手机提供） | FR-4（仅查看/归档）、FR-6, FR-13, FR-26, FR-27, FR-28, FR-34, FR-37, FR-39 |
| 引擎持有（仅状态可见） | FR-10, FR-25, FR-31, FR-32, FR-35, FR-36 |

> 2026-09-10 产品裁决：手机端不提供空状态引导流程（FR-21 移动侧退役，桌面引导保留），初始设置统一在桌面完成；spec-mobile-fr-parity 组 6 相应失效。

#### FR-40: 桌面配对与连接管理

用户通过一次性配对将手机与桌面端绑定。同一局域网内自动发现并直连桌面引擎；离开局域网自动切换经云中继的加密转发通道，返回后自动切回直连。连接状态在手机端持续可见（局域网直连/中继转发/离线），中继只转发端到端加密流量、不落地存储任何用户数据。 `[ASSUMPTION: V1仅支持绑定一台桌面设备]` `[ASSUMPTION: 中继仅为加密流量转发]`

**Consequences (testable):**
- 首次使用通过扫码或配对码完成绑定，无需手动输入网络配置
- 同一局域网内自动发现并直连桌面引擎，无需人工干预
- 离开局域网自动经云中继保持连接，回到局域网自动切回直连
- 手机端任意时刻可查看当前连接状态（直连/中继/离线）
- 配对关系持久化；重装App后重新配对即可恢复，无需桌面端重置
- 中继无法读取传输明文

#### FR-41: 实时状态同步

桌面端的角色/管家活动、任务推进状态、仪表盘指标实时呈现于手机端，状态变化主动推送刷新，核心状态界面无需手动下拉刷新。手机端发起的对话、建议确认/拒绝、任务操作均作为指令交由桌面引擎执行，执行结果实时回流显示；锁屏通知上的快捷操作与App内操作等效。断线重连后以最新快照补齐状态。 `[ASSUMPTION: 重连仅补最新快照，不做历史增量回放]`

**Consequences (testable):**
- 角色/管家正在执行的活动（含委派任务进行中）在手机端即时更新
- 四象限任务进度、大石头保护状态、仪表盘指标实时反映桌面最新状态
- 手机端对话由桌面引擎执行，流式结果回流至手机展示
- 锁屏通知上的确认/拒绝操作与App内ActionCard操作等效生效
- 断线期间桌面的状态变化在重连后以最新快照呈现

#### FR-42: 移动推送通知 `[DEFERRED — 已移出 V1，见下方修订说明]`

> **2026-08-25 架构阶段修订**：经产品负责人裁决本需求整体降级出 V1。原因：FCM HTTP v1 需 service account 凭证，无法安全分发至每个用户的桌面端；备选"中继内置 FCM 代理"因需放宽「中继只做端到端加密转发、零知识」隐私假设被否决。V1 仅提供应用内通知；架构已预留 NotificationDispatch 抽象层（见 architecture.md 手机伴侣增量章节），FCM 中继代理 / UnifiedPush 作为后续可插适配器。以下原始验收条款在推送重新进入范围时恢复生效。

三级通知映射为独立的Android系统通知渠道，用户可按级别分别开启/关闭；主动建议类通知支持在系统通知上直接确认/拒绝，等效于App内ActionCard。点击通知深链直达对应内容（晨间简报、建议卡、任务详情）。通知每日上限与"沉默原则"（见Non-Goals）同样约束推送通道。

**Consequences (testable):**
- 三级通知各自对应独立系统通知渠道，可按级开关
- 主动建议通知可在锁屏直接确认或拒绝，结果与App内操作等效
- 点击通知无中间层直达对应内容页
- 推送频率遵守既有的每日上限与沉默原则
- 桌面应用未运行时不产生新推送（工作循环依赖桌面运行）

#### FR-43: 离线降级与速记排队

桌面引擎不可达时手机进入降级态：展示最后已知状态的只读缓存，明确标注数据截止时间；仅开放快速文字速记入口，速记在恢复连接后自动提交管家处理。降级期间所有依赖引擎的操作入口不可交互并说明原因，避免产生无法执行的操作。 `[ASSUMPTION: 离线缓存限于最后已知快照+文字速记]`

**Consequences (testable):**
- 离线时界面明确标注离线与数据截止时间，不以陈旧数据冒充实时的
- 降级态下可进行文字速记，重连后自动入队交管家处理且无丢失
- 依赖引擎的功能入口在降级态不可交互并说明原因
- 局域网内桌面关机与广域网中断呈现统一的降级体验

> 2026-09-08 产品裁决：降级呈现采用非阻断紧凑状态指示（SPEC-companion-connection-chat-ux），全屏阻断遮罩范式废止；连接期引入 Connecting/Reconnecting 宽限态；离线文字录入由速记条改为对话输入离线待发（网络不可用仅提示，恢复后自动续发）。

### 4.15 云端托管版（自托管）

EgoSync 的云端部署形态为**自托管云端版**：完整引擎（Rust 编排层 + opencode + SQLite）与全部用户数据运行于用户自有服务器（VPS/NAS），以 Docker 部署，WEB 浏览器为主要客户端。数据拓扑为**云脑单源**——引擎与数据只在云端实例运行一份，客户端多端访问，不做双引擎同步。形态限定为自托管单用户，无多租户。本赛道于 V1 收尾后启动（Epic 15-17：引擎无头化 → WEB 客户端 → 自托管部署运维），V1 MVP 范围不变；桌面版与手机伴侣（§4.14）既有规划不受影响。

#### FR-44: 云端部署形态

用户可在自有服务器（VPS/NAS）以 Docker 一键部署 EgoSync 云端版，运行完整引擎（Rust 编排层 + opencode + SQLite）。单实例单用户，无多租户。同一用户同一时刻只有一个事实源实例（桌面版或云端版二选一持有数据，另一形态作为客户端访问，见 FR-48）。 `[ASSUMPTION: 云端版为单实例单用户自托管；同一用户同一时刻仅一个事实源实例；不做多实例间数据同步]`

**Consequences (testable):**
- 用户在自有 VPS/NAS 上通过 Docker 完成部署，部署产物包含完整引擎：Rust 编排层（server binary）、opencode、SQLite 数据库
- 部署后无需安装任何客户端应用，即可通过 WEB 浏览器使用（FR-45）
- 实例以单用户模式运行，不提供多租户/多账号隔离能力（多租户维持 Non-Goal，见 §5）
- 服务器重启后实例自动恢复运行，用户数据与配置完整无丢失
- 桌面版与云端版可各自独立部署使用；任一时刻用户数据只存在于一个事实源实例中

#### FR-45: WEB 客户端

任意现代浏览器（桌面/移动，含 iOS Safari）可访问云端实例，覆盖核心体验功能集（对齐手机伴侣一档核心 FR 的口径，具体分档由 UX/架构阶段确定）。流式对话、任务操作、建议确认、仪表盘、简报复盘均可用。WEB 客户端与桌面版共用同一 React 组件体系。 `[ASSUMPTION: 核心体验功能集的具体分档清单由 UX/架构阶段确定]`

**Consequences (testable):**
- 桌面浏览器与移动浏览器（含 iOS Safari）均可登录云端实例并完成核心操作，无需安装任何应用
- 流式对话、任务操作、建议确认/拒绝、仪表盘、晨间简报与周复盘在 WEB 客户端均可用
- 对话流式输出在浏览器中实时渲染，体验与桌面版一致（同一 React 组件体系，视觉零分叉）
- 浏览器刷新或断线重连后恢复会话状态，不丢失已产生的消息
- 移动浏览器（<768px 视口，含 iOS Safari）呈现移动专用形态：底部 tab 导航替代侧栏、全屏对话、触控目标 ≥44px、刘海/底部安全区避让；≥768px 桌面布局零变化（2026-09-25 补充，Epic 16 Story 16.4）
- 云端实例可安装为主屏应用（PWA）：manifest / 图标 / 应用外壳离线缓存就位；业务数据不落浏览器（刷新仍从服务端重取）（2026-09-25 补充，Epic 16 Story 16.4）

#### FR-46: 云端实例访问认证

自托管无注册体系；单用户访问凭据（初始化时设置令牌/passkey），全部端点强制认证，传输加密（TLS）。LLM API Key 等服务端密钥由服务端密钥管理承载，不经任何 API 暴露给浏览器。 `[ASSUMPTION: 自托管无账号注册体系，凭初始化凭据访问]`

**Consequences (testable):**
- 首次部署初始化时设置单用户访问凭据（令牌/passkey），全程无注册/多账号流程
- 除健康检查等运维必需端点外，全部 API 端点强制认证；未认证请求被拒绝且不泄露任何用户数据
- 客户端与实例间全部通信经 TLS 加密（内置或反向代理终结）
- LLM API Key 等服务端密钥仅存在于服务端（env/加密文件），任何 API 响应不将其下发给浏览器客户端

#### FR-47: 云端常驻工作循环

云端版中，角色工作循环随服务器常驻运行（7×24），晨间简报/周复盘按计划时间生成，不依赖任何客户端在线。桌面版的"应用运行时执行"假设（FR-10）在云端版中被服务器常驻取代；建议仍遵循"需用户确认"原则（FR-11），常驻生成不改变交互边界。 `[ASSUMPTION: 云端版工作循环依赖服务器常驻运行；服务器停机期间无新建议产生（诚实代价）]`

**Consequences (testable):**
- 工作循环按配置频率随服务器常驻执行，无任何客户端在线时照常运行
- 晨间简报/周复盘在计划时间自动生成并留存，用户下次访问时可见
- 服务器停机期间无新建议产生；服务器恢复后工作循环按计划继续执行
- 常驻生成的新建议仍以待确认状态呈现，用户确认后才转为正式任务（FR-11 边界不变）
- 本 FR 不改变桌面版 FR-10 的行为——桌面版工作循环仍在应用运行时执行

#### FR-48: 桌面客户端远程模式（可选，后置）

桌面应用可切换为连接云端实例的客户端使用（本地引擎待机），实现"同一份数据、双形态访问"。依赖 Epic 15 的传输对等基建，作为云端赛道第二阶段交付；不交付不影响 FR-44 至 FR-47 的完整性。

**Consequences (testable):**
- 桌面应用可切换至远程模式连接云端实例作为客户端使用；本地引擎待机，不产生本地数据副本
- 远程模式与本地模式可切换；切换为连接目标变更，不触发任何数据合并或同步操作（数据仅在云端实例，单源语义）
- 交付时序位于 Epic 15 传输对等基建之后，作为云端赛道第二阶段交付；本 FR 为可选后置需求，不交付不阻塞云端版首版验收
...
- 手机伴侣App（Android，§4.14）：连接桌面引擎的移动视图与指令入口——FR-40至FR-43（FR-42 推送已降级出 V1，仅应用内通知），及一档18项核心FR＋二档6项适配FR的移动呈现（2026-09-10 裁决：FR-21 移动侧引导退役，核心 FR 计 17 项——spec-companion-android-remove-onboarding）

### 6.2 Out of Scope for MVP

- **移动端独立运行/全功能对等** — V1移动形态限定为伴侣App（§4.14）；手机本地引擎与iOS延期 `[NON-GOAL for MVP]`
- **PWA Web 推送（Service Worker 推送）** — iOS Safari 16.4+ 仅限加主屏 PWA 支持；与架构「WEB 端应用内通知」裁决一致（页面关闭即无通知）。PWA 其余能力（manifest/图标/外壳缓存）已纳入范围（FR-45，Story 16.4） `[NON-GOAL for MVP]`
- **云端数据同步/托管** — 云端托管版（自托管，云脑单源，见 §4.15）已立项为 V1 收尾后的独立赛道；不在 V1 MVP 内 `[NON-GOAL for MVP]`；多租户托管仍为 Non-Goal（见 §5）。（注：原"端到端加密云同步层"（桌面版多设备同步）与云端版单源模型语义重叠，V2 规划时重新评估二者关系）
...
- `[ASSUMPTION]` §4.14 / FR-40: V1仅支持绑定一台桌面设备；中继只做端到端加密转发、不落地存储用户数据，部署方案由架构阶段确定
- `[ASSUMPTION]` §4.14 / FR-41: 断线重连仅补发最新快照，不做历史消息增量回放
- `[ASSUMPTION]` §4.14 / FR-42: ~~推送依赖Android系统通知渠道下发，不自建常驻推送服务~~ **已被 2026-08-25 架构阶段修订取代**：FR-42 整体降级出 V1，推送通道延后（NotificationDispatch 抽象层保留）
- `[ASSUMPTION]` §4.14 / FR-43: 离线缓存限于最后已知快照+文字速记，不支持离线浏览完整历史对话
- `[ASSUMPTION]` §4.15 / FR-44: 云端版为单实例单用户自托管；同一用户同一时刻仅一个事实源实例；不做多实例间数据同步
- `[ASSUMPTION]` §4.15 / FR-46: 自托管无账号注册体系，凭初始化凭据访问
- `[ASSUMPTION]` §4.15 / FR-47: 云端版工作循环依赖服务器常驻运行；服务器停机期间无新建议产生（诚实代价）
...
- V1基线零服务器成本（纯本地+局域网直连）；伴侣模式远程访问经云中继转发，是首个服务端组件 `[ASSUMPTION: 中继部署与成本方案由架构阶段确定]`
- BYOK模式下LLM成本由用户自控
...
- **V1**：Android伴侣App——连接桌面引擎的远程视图与指令入口（§4.14），仅Android
- **V2**：iOS伴侣App、云端同步层（端到端加密）
