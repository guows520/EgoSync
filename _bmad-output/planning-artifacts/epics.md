...
本文档基于 PRD（39 个 FR）、Architecture（Tauri 2.x + React 18 + Rust 后端 + opencode sidecar 架构）、UX Design Specification（"宁静书房"布局 + shadcn/ui 设计系统）和**已实现的高保真前端原型 `egosync-app/src/App.tsx`（1458 行，21 个组件）**，将需求分解为可由开发者执行的 Epic 和 Story 列表。
...
**4.13 Skill 指定、管家统计与 MCP 管理（2026-07-23 增量）**
- **FR-37**: 对话中 @Skill 指定 — 管家和角色可从当前 Agent 已添加且启用的 Skill 中选择本轮任务使用的 Skill；选择仅作用于当前任务；无效选择在 Runtime 前拒绝并提示；未指定时保持自动发现行为；实际使用 Skill 可审计
- **FR-38**: 管家仪表盘统计与筛选 — 聚合任务、结构化记忆、对话会话和待处理任务四项指标，支持 Agent 与时间范围组合筛选，并统一时间字段和计数语义
- **FR-39**: MCP Server 启停与管家独立绑定 — 支持 Server 生命周期管理和管家独立绑定；Server 状态、管家绑定、角色绑定相互独立；关闭时保留绑定但停止运行时工具暴露；刷新失败显式报告
...
- **NFR-14 分层通信**：React 仅经 Tauri IPC，Command 仅解析参数，业务规则位于 Rust Service，数据访问位于 Repository/DB
- **NFR-15 任务快照一致性**：Skill/MCP 可用集合在任务启动前验证并冻结，流式执行期间不改变本次上下文
- **NFR-16 运行时一致性**：数据库事实、`opencode.json` 投影与 sidecar 运行状态必须返回明确同步结果
- **NFR-17 安全校验**：`@Skill` 不得绕过 Agent 作用域和启用校验；关闭的 MCP Server 不得因旧投影继续暴露工具
- **NFR-18 可审计性**：显式 Skill、Dashboard 筛选条件与 MCP Runtime 刷新结果可追踪
- **NFR-19 显式失败**：配置持久化成功但 Runtime 刷新失败时，不得静默宣称完全成功
- **NFR-20 并发与幂等**：MCP mutation 与 Runtime refresh 复用现有锁并支持串行/合并，重复刷新不残留或复制工具
...
**FR-37～FR-39 增量架构约束：**
- 不重新设计 Skill、Dashboard 或 MCP 子系统，不升级基础依赖
- Agent 身份由后端可信上下文解析；前端不得提交未经验证的可用 Skill 集合
- Dashboard 使用统一 Tauri Command 和聚合 DTO；会话统计通过现有 Repository/Adapter 边界接入
- 数据库是配置事实来源，`opencode.json` 仅由 `AgentConfigService` 维护，sidecar 是运行状态
- MCP CRUD、enabled、绑定、配置投影与 Runtime refresh 复用现有链路和 `OpencodeMcpScopeLock`
- Skill 长期配置与请求级 `selectedSkillId` 使用不同语义和数据结构
- UX 规范未定义本增量的具体控件；实现应遵循现有仪表盘视觉优先、对话常驻、键盘可操作原则，不虚构额外视觉方案
...
### Epic 10: 可控的 Agent 能力调用与工具管理

用户可以在管家或角色对话中明确选择本轮使用的 Skill，并能管理 MCP Server 及管家的独立绑定；系统始终只向当前 Agent 暴露其被授权且处于启用状态的能力。

**FRs covered:** FR-37, FR-39

**NFRs covered:** NFR-14, NFR-15, NFR-16, NFR-17, NFR-18, NFR-19, NFR-20

**核心交付：**
- 请求级 `@Skill` 候选、选择、后端校验、Runtime 注入、审计与错误反馈闭环
- MCP Server 查看、添加、编辑、测试、删除和启停
- 管家 MCP 独立绑定及其与角色绑定的隔离
- enabled 与绑定交集形成运行时有效工具集合
- 配置投影、串行/幂等 Runtime refresh 与部分失败提示
- 复用现有 Skill、MCP、`AgentConfigService` 和 `OpencodeMcpScopeLock` 边界

---

### Epic 11: 可筛选的跨 Agent 活动洞察

用户可以在管家仪表盘查看全部或指定 Agent 在选定时间范围内的任务、结构化记忆、对话会话与待处理任务统计，快速理解整体活动状况。

**FRs covered:** FR-38

**NFRs covered:** NFR-14, NFR-18, NFR-19

**核心交付：**
- 四项指标统一计数语义和聚合 DTO
- 全部、管家、单个角色与时间范围组合筛选
- 任务/记忆 `created_at`、会话 `started_at` 的统一时间边界
- 会话 Repository/Adapter 跨数据源接入
- 默认全部时间，以及加载、空数据和失败状态
- 复用现有 Dashboard Command、Service、Repository 与前端 Dashboard
...
## Epic 9: 设置、显示与 Windows 体验加固（Settings, Rendering & Windows UX Hardening）

在不改变既有业务架构和后端 Prompt 的前提下，收敛设置页信息架构、角色创建目标、Markdown 表格、局部深色模式、前端术语、任务时间控件以及 Windows 安装器语言问题。该 Epic 来源于 2026-07-22 的专项调查和用户确认边界。

### Story 9.1: 管家与角色设置分组支持折叠和批量展开

支持一级设置分组独立折叠、全部展开/折叠、状态持久化、键盘语义，并保持所有既有保存事务和 Skill 二级状态不回归。

### Story 9.2: 角色设置结构优化并在新建角色时填写目标

新增角色目标输入；将自定义 Skill 纳入 Skill 配置；把角色信息保存按钮移入角色信息分组；移除底部统一保存按钮；将“外部 MCP 工具”改为“MCP Server配置”。

### Story 9.3: 对话消息正确渲染标准 GFM Markdown 表格

接入 `remark-gfm` 并提供可横向滚动、支持深色模式的表格渲染；明确不兼容 TAB 表格，不引入图表库，不修改 Rust Prompt。

### Story 9.4: 补齐局部深色模式并统一“数字分身管家”文案

修复建议卡片和顶部导航选中态的深色模式；将前端可见“分身管家”更新为“数字分身管家”，Rust Prompt 保持不变。

### Story 9.5: 任务时间选择完成后自动收起并将 Windows 安装包默认语言设为中文

先在 Windows WebView2 验证原生时间控件，必要时替换为可控 picker；同时配置并实际验证 MSI/WiX 与 NSIS 两种安装包默认简体中文。

## Epic 10: 可控的 Agent 能力调用与工具管理

用户可以在管家或角色对话中明确选择本轮使用的 Skill，并在管家设置中维护独立的 MCP Server 绑定；系统始终只向当前 Agent 暴露其获准且启用的能力。
...
**Given** 用户提交不存在的角色、开始晚于结束、无法解析或不完整的时间范围
**When** 后端验证
**Then** 请求被拒绝并返回明确错误
**And** 不执行部分聚合
**And** 前端保留上一次成功结果。
...
- **16.4** 移动端 WEB 形态与 PWA 增强——<768px 侧栏退场改底部 tab 导航 + 全屏对话 + 44px 触控目标 + 安全区补全 + manifest/maskable 图标/Service Worker 应用外壳缓存（业务数据零缓存，NFR-C3 不变）+ CSP worker-src/manifest-src 增量 + 移动视口 e2e；结清 16.2 延期的「UX 定稿级移动形态」与 UX-C4 断点细节欠账（2026-09-25 Correct Course 批准，sprint-change-proposal-2026-09-25.md）
...
15.1 → 15.2 → 15.3 → 15.4 → 15.5
  ...
  → 16.4（依赖 16.2 响应式基线 + 16.1 静态服务；与 16.3 互不依赖；与 Epic 17 并行无冲突——2026-09-25 Correct Course 批准）
（16.3 不阻塞云端版首版验收——FR-48 可选后置语义）
（16.4 建议作为 Epic 16 → done 前置——FR-45「移动浏览器（含 iOS Safari）」验收需逐条过）
```
...
| FR-45 WEB 客户端 | E16（16.1–16.4）+ E15（15.5 传输对等基建） | 浏览器双端核心操作 / 流式实时渲染体验一致 / 刷新重连恢复状态 / 移动端专用形态 / PWA 可安装——逐条落位 |
...
| UX-C1～C4 | 16.1（首屏/探测/门控/TitleBar）/ 16.2（流式/重连/响应式基线）/ 16.4（断点与移动交互细节定稿——UX-C4 欠账结清，2026-09-25 Correct Course）；UX 章节定稿后如有新约束以勘注补入 |
...
**来源：** sprint-change-proposal-2026-09-25.md（2026-09-25 批准）
