...
![BA、健康管理和父亲三个角色各自独立且可追溯的记忆](./assets/egosync-role-memories.png)

*三个角色分别沉淀事实、偏好和认知模式；点击“查看原文”，可以追溯每条记忆来自哪段对话。*

**第二，它不只会聊天，还会推进事情。** 你可以让管家拆解并分发任务，也可以直接进入某个角色对话。任务会进入统一的管理视图，并根据目标、期限和上下文进入四象限，而不是要求你为每件事机械地标注优先级。

![数字分身管家将文章、跑步和亲子活动分别委派给对应角色](./assets/egosync-task-delegation.png)

*同一周的安排由数字分身管家识别和拆分，再分别交给 BA、健康管理与父亲角色继续推进。*

**第三，每个角色都可以拥有不同能力。** 管家和角色能够分别配置 Skill，也可以通过 MCP 接入外部工具。你可以让“研究者”擅长检索与整理，让“内容创作者”专注写作，而不是给所有 Agent 塞入同一套臃肿能力。

![BA、健康管理和父亲角色分别配置不同的 Skill](./assets/egosync-role-skills.png)

*BA、健康管理和父亲拥有各自的专业 Skill 套件，而不是共享一套臃肿、缺少角色边界的通用能力。*
...
### Logo 里藏着 EgoSync 的产品理念

![EgoSync Logo](./assets/egosync-logo.png)

EgoSync 的logo，是一组**围绕同一中心、彼此衔接的双向循环**。我更愿意把它理解为产品理念的一次视觉化表达：

- **中央的圆点代表“Ego”——用户自己。** 无论我们拥有多少身份、目标和 AI 分身，中心始终是同一个人；所有角色最终都应服务于同一份人生使命，而不是各自为战。
- **圆角六边形代表每个人的多重角色分身。** 六边形的不同侧面，如同一个人在工作、健康、家庭和自我成长等场景中的不同角色；它们各有目标与职责，却共同构成一个完整的“我”。
- **首尾衔接的双箭头代表“Sync”——自我与 EgoSync 之间的持续双向同步。** 你把目标、任务、偏好和反馈交给 EgoSync，EgoSync 则通过理解、记录、执行与建议持续回应你；它不是单向接收指令的工具，而是在一次次互动中与你共同演进。
- **靛紫色传递出科技、理性与可信赖感。** 它也提醒我，EgoSync 追求的不是一个喧闹的效率机器，而是一位能够长期陪伴、冷静协调的数字分身管家。

合在一起，这个 Logo 想表达的不是“多个互不相干的 AI 助手”，而是：**多个角色分身始终围绕同一个自我运转，并通过持续同步，逐渐成为一支理解你、协助你的个人 AI 团队。**
...
在开始下面的操作之前，可以先用下图认识 EgoSync 的整体布局。左侧以角色组织不同分身，顶部进入当前空间的仪表盘、任务、记忆与设置，中央用于对话和查看执行过程，底部则负责输入需求。管家与角色分身共用一致的操作框架，切换角色时不需要重新学习一套界面。

![EgoSync 主界面各功能区域说明](./assets/egosync-main-interface-annotated-v2.png)

*EgoSync 主界面以数字分身管家为全局入口，通过角色切换、功能导航、对话执行和消息输入等区域，将多个分身组织在同一个个人工作空间中。*

### 预置步骤：配置大模型服务

在使用自然语言创建角色、与数字分身对话或让管家分发任务之前，需要先为 EgoSync 配置一个可用的大模型服务。EgoSync 采用 BYOK（自带 API Key）方式，公司内部可以使用“昇腾集成验证”提供的大模型服务。

1. 点击左侧边栏底部的**设置**按钮，进入**全局设置 → 模型服务配置**。
2. 点击**添加新配置**，填写配置名称，并选择提供商。当前界面支持 OpenAI 兼容、Anthropic 兼容、MiniMax、智谱、DeepSeek、Kimi 和阿里云百炼；选择提供商后，系统会自动带出默认 API 地址，也可以根据实际服务进行修改。
3. 填写 **API 密钥**和**模型名称**。模型名称可以直接输入；也可以先填写 API 密钥，再点击**获取模型列表**，从服务商返回的可用模型中选择。
4. 根据服务所在位置选择网络方式：公网模型选择**外网（走系统代理）**，企业内网或本地服务选择**内网（直连，绕过代理）**。
5. 点击**保存配置**。返回配置列表后，点击**测试连接**；看到“连接成功”，说明该配置可以正常调用。如果配置了多个模型服务，通过配置名称左侧的单选按钮选择当前启用的服务。

**预期结果：**配置列表中会显示模型名称、提供商和网络方式；被选中的配置会标记为“当前启用”，测试后显示“连接成功”。完成这一步后，再继续下面的角色创建和任务委派流程。
...
注：本文及配图由我给大纲，AI基于PRD等材料出初稿，再由我和AI交互，二次编辑而成