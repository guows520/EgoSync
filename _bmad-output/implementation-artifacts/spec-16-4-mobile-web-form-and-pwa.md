...
- 设置 tab = 桌面 GlobalSettingsModal 同款全量（模型服务 / MCP Server / 调度时间 / 数据与隐私 / 主题 / 登出，打开同一批组件不重写）；远程模式/手机伴侣 desktop-only，浏览器宿主经 capabilities 门控自动隐藏；角色级设置（角色信息/主动性/技能/MCP）保持在角色详情内的设置 tab。〔2026-09-26 owner 授权修订：「通知」入口已移除——此处原列 7 项含「通知」，与 Spec Change Log 同日条目「交付后增量：移动自适应五症状修复」症状③重协商一致；通知声音配置经「调度时间」→ scheduler tab 可达〕
...
- **触发**：用户口述 web 版手机浏览器（<768px）五症状（根因与证据链见案卷 `investigations/web-mobile-adaptive-issues-investigation.md`，五症状代码级根因全部 Confirmed）。其中②（ActionCard 小屏「拒绝/确认」全宽纵排过大）与③（移动设置「通知」行与「调度时间」落点完全重复，桌面本就无该项）触及 2026-09-25 冻结设计（冻结块「设置 tab = 桌面 GlobalSettingsModal 同款全量（……/通知/主题/登出）」+ epic-16-context「ActionCard 全宽堆叠」+ ActionCard.mobile.test.tsx / web-mobile.spec.ts 既有钉孔）；④（LLM/MCP 卡片「当前启用/测试连接」等小屏挤压换行）为 flex squeeze 真缺陷（MCP 卡同族）；①（共享 Modal 无高度/滚动，小屏超高内容裁切且不可滚）与⑤（ChatStream `scroll-smooth` 把切换视图后的挂载贴底动画化）非冻结缺口。owner 2026-09-26 三项裁决：②③**授权解冻**（可按新行为改写既有钉孔并留本记录）、①走**共享修复**（Modal.tsx 补 max-md 高度/滚动，方案 A）、④确认「4 个图标」映射（徽章缩图标 + 测试连接/编辑/删除 3 图标钮，max-md: 切换、≥44px 触控、可访问名保留）。
- **改动**（硬红线：桌面 ≥768px 逐像素零变化，只加 `max-md:`/`md:` 类对）：① `Modal.tsx:61` 对话框类追加 `max-md:max-h-full max-md:overflow-y-auto`——8 个使用方一次覆盖（含症状本体 AddRoleModal 与潜伏同症的 TaskModal；WeeklyReviewModal/ButlerSettingsContent/RoleConfirmModal 自带 max-h 不变）；遮罩安全区 pt/pb 类严禁动（Modal.safearea.test.tsx 钉死）。② `ActionCard.tsx` 按钮容器 → `flex flex-wrap justify-end gap-2 mt-3`、两钮去 `max-md:flex-1`（保留 `max-md:min-h-[44px]` 触控红线；375px 下「拒绝+确认」一行放得下；`flex-wrap` 为调查卷推荐项+评审 patch，当前标签宽度下 inert，兜未来自定义标签）。③ `MobileSettingsView.tsx` 删「通知」行（Bell import 同步删——唯一引用点），内容入口收敛为 llm/mcp/scheduler/data 四项；敲门通知声音经「调度时间」→ scheduler tab 仍可达（GlobalSettingsModal.tsx:1039-1068），功能零损失。④ `GlobalSettingsModal.tsx` LLM/MCP 两卡图标化：测试连接/编辑/删除 → 单按钮 + 文字 span `max-md:sr-only` + 图标 `md:hidden`（Zap/Pencil/Trash2），按钮加 `max-md:h-11 max-md:w-11 max-md:p-0 max-md:flex max-md:items-center max-md:justify-center`；「当前启用」徽章缩 Check 图标（`md:hidden`）+ 文字 `max-md:sr-only`；**loading 分支保持纯 Loader2**（GlobalSettingsModal.test.tsx:262 钉死）；MCP 开关 toggle 不动。⑤ `ChatStream.tsx` 症状⑤最终形态（评审 patch，路径 b）：容器 `:1354` **保留** `scroll-smooth`；`:881-886` 挂载/更新贴底 effect 赋值前临时置 `style.scrollBehavior='auto'`、赋值后恢复——贴底即时落底（owner 口径达成），来源消息居中（:238 显式 smooth + :241 修正赋值）与滚轮平滑全部保留。初版「删 scroll-smooth」被三层评审证伪（:241 赋值由 CSS 平滑化变即时会抢占 :238 动画，来源导航居中动画静默丧失），修订记录见 `spec-web-mobile-adaptive-fixes.md` Review Triage Log F4/F5；新建 `ChatStream.scroll.test.tsx` 钉「类保留 + auto→赋值→恢复序列」。
- **测试**：新建三个钉孔文件（`Modal.scroll.test.tsx`、`MobileSettingsView.rows.test.tsx`、`GlobalSettingsModal.mobileIcons.test.tsx`——遵守冻结规矩「新测试一律新建文件」）；owner 授权范围改写既有钉孔三处——`ActionCard.mobile.test.tsx` 两例（无 `max-md:flex-1` / 容器 `justify-end` 且无 `max-md:flex-col`、`max-md:justify-stretch`）、`web-mobile.spec.ts` 通知相关钉孔（用例标题 7→6 项入口、注释改「4 项内容入口」、key 数组去 `'notification'`、删通知落点断言块）、以及 owner 2026-09-26 追加授权的 `App.mobile.test.tsx:201/208`（清点时漏点的第三处通知钉孔——initialTab 落点守门用例；同型改写为 `settings-row-scheduler` 在场 + 点击，`data-initial-tab='scheduler'` 落点断言保留，6/6 全绿）。守门语义口径（评审 patch F7 校正）：直接映射守门（行点击 → initialTab 落点）保留；跨映射别名守门（原 notification→scheduler）随③的通知别名移除而消失——别名已不存在可守护对象，非测试弱化代码缺陷。既有 GlobalSettingsModal.test.tsx / ActionCard.test.tsx / Modal*.test.tsx / MobileSettingsView.browser.test.tsx 零改动全绿；web-resident-loop.spec.ts:271 只查「拒绝」文本在场零影响。
