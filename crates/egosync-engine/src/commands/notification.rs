//! notification 域命令体（Story 15.4 自壳 `commands/notification.rs` 平移，
//! 业务逻辑零改动；State/AppHandle 取值改 `&EngineCtx`，emit 改总线+常量；
//! 内联测试随迁）。
...
use crate::events::NOTIFICATION_READ_EVENT;
...
pub async fn notification_create(
    ...
    let (notification, payload) = create_and_build_payload(&ctx.pool, &input).await?;

    // 原 emit 错误映射后 `let _ =` 丢弃（静默语义保持）
    let _ = serde_json::to_value(&payload)
        .map_err(|e| e.to_string())
        .and_then(|value| ctx.bus.emit(NOTIFICATION_NEW_EVENT, value));

    Ok(notification)
}

/// 列出所有通知（带角色信息），按创建时间倒序。
pub async fn notification_list(ctx: &EngineCtx) -> Result<Vec<NotificationWithRole>, AppError> {
    db::notifications::list_notifications(&ctx.pool).await
}
...
pub async fn notification_mark_read(
    ...
    // Story 13.1 评审决策①：已读须触发 STATE_DELTA，否则手机未读角标永远滞后。
    let notification = db::notifications::mark_read(&ctx.pool, &id).await?;
    if let Err(e) = serde_json::to_value(&notification)
        .map_err(|e| e.to_string())
        .and_then(|value| ctx.bus.emit(NOTIFICATION_READ_EVENT, value))
    {
        tracing::warn!(event = NOTIFICATION_READ_EVENT, error = %e, "notification 写事件发射失败");
    }
    Ok(notification)
}

/// 统计未读通知数量。
pub async fn notification_count_unread(ctx: &EngineCtx) -> Result<i64, AppError> {
    db::notifications::count_unread(&ctx.pool).await
}
