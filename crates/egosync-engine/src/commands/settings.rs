//! settings 域命令体（Story 15.4 自壳 `commands/settings.rs` 平移，
//! 业务逻辑零改动；State 取值改 `&EngineCtx`；内联测试随迁）。

use crate::commands::ctx::EngineCtx;
use crate::db::app_settings;
use crate::db::pool::DbPool;
use crate::error::AppError;
// Story 15.2：8 个调度常量已下沉 engine scheduler（值不变），此处直引
...
pub async fn settings_get_schedule(ctx: &EngineCtx) -> Result<ScheduleConfig, AppError> {
    read_schedule_config(&ctx.pool).await
}
...
pub async fn settings_update_schedule(
    ...
    // 校验全部通过后再统一写入
    if let Some(ref v) = briefing_time {
        app_settings::set_setting(&ctx.pool, KEY_BRIEFING_TIME, v).await?;
    }
    if let Some(ref v) = review_day {
        app_settings::set_setting(&ctx.pool, KEY_REVIEW_DAY, v).await?;
    }
    if let Some(ref v) = review_time {
        app_settings::set_setting(&ctx.pool, KEY_REVIEW_TIME, v).await?;
    }
    if let Some(ref v) = bigrock_reminder_day {
        app_settings::set_setting(&ctx.pool, KEY_BIGROCK_REMINDER_DAY, v).await?;
    }
    if let Some(ref v) = bigrock_reminder_time {
        app_settings::set_setting(&ctx.pool, KEY_BIGROCK_REMINDER_TIME, v).await?;
    }
