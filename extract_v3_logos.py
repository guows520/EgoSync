...
def process_v3_logos():
    # 找到最新的 V3 绿幕截图
    pattern = r'C:\Users\Admin\.gemini\antigravity\brain\28c43195-75a0-432a-91b2-38e02651be47\v3_twin_logos_green_*.png'
    files = glob.glob(pattern)
    if not files:
        print("未找到 V3 版本绿幕截图", flush=True)
        return
        
    file_path = files[-1]
    print(f"载入 V3 截图: {file_path}", flush=True)
    ...
    if left_non_greens:
        ...
        print(f"检测到左侧 V3 透明版大小: {side}x{side}", flush=True)
        crop_and_key_fast(img_pixels, left_b, top_b, right_b, bottom_b, "logo_v3_transparent")
    else:
        print("未在左半边检测到 V3 Logo，截图有可能出现白屏或绿屏遮罩", flush=True)
    ...
    if right_non_greens:
        ...
        print(f"检测到右侧 V3 有底色版大小: {side}x{side}", flush=True)
        crop_and_key_fast(img_pixels, left_b, top_b, right_b, bottom_b, "logo_v3")
    else:
        print("未在右半边检测到 V3 Logo，截图有可能是残缺页", flush=True)
