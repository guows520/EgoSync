...
pub fn run() {
    ...
    let builder = if remote_mode {
        // [T18 修订] 远程 builder 补 .setup：仅 Windows DWM 边框修复（与
        // 本地分支同款——远程态同为桌面壳窗口，无边框窗口左/下/右黑色
        // 边线的回归预防）+ 即刻返回；仍零引擎装配（不打开任何数据库、
        // 不启 sidecar/调度器/双 watch——模式裁决已在 builder 分支完成）。
        tauri::Builder::default()
            .setup(|app| {
                ...
                // 非 Windows 宿主：闭包参数标记使用（cfg 块外零引用面）
...
mod tests {
    ...
    fn remote_mode_skips_engine_assembly_before_db_open() {
        ...
        //
        // [T8 修订] 精确匹配守卫语句（`if remote_mode { return Ok(()); }`），
        // 无 `.or_else` 兜底——守卫删除/重排时 find 返回 None ⇒ 测试红
        //（原 `.or_else(|| source.find("if remote_mode {"))` 会在守卫删除
        // 后回退匹配更早的 tracing 块，断言被掏空）。
        let source = lib_source();
        let guard = source
            .find("if remote_mode {\n                return Ok(());")
            .expect("setup 内远程守卫存在（remote_mode ⇒ return Ok(()) 须先于 init_db）");
        let db_init = source
    ...
    #[test]
    fn remote_builder_setup_is_dwm_fix_only() {
        // WHY（[T18 修订]）：远程 builder 须补 .setup——仅 Windows DWM
        // 边框修复（远程态同为桌面壳窗口）+ 即刻返回 Ok(())，仍零引擎
        // 装配（片段内不得出现任何数据库/引擎装配调用）。
        let source = lib_source();
        let remote_branch = source
            .find("let builder = if remote_mode {")
            .expect("模式二选一 builder 分支存在");
        ...
        let remote_fragment = &source[remote_branch..local_branch];
        assert!(
            remote_fragment.contains(".setup(|app| {"),
            "远程分支须有 .setup（Windows DWM 边框修复载体）"
        );
        assert!(
            remote_fragment.contains("set_shadow(false)"),
            "远程 setup 须含 DWM 边框修复（与本地分支同款）"
        );
        assert!(
            !remote_fragment.contains("init_db"),
            "远程 setup 零引擎装配（不打开数据库）"
        );
        assert!(
            !remote_fragment.contains("spawn_scheduler"),
            "远程 setup 零引擎装配（不启调度器）"
        );
    }
}
