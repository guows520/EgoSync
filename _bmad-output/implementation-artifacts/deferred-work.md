...
- source_spec: `_bmad-output/implementation-artifacts/spec-16-4-mobile-web-form-and-pwa.md`
  summary: 【已收口（16.4 交付后增量，2026-09-25）】移动形态无角色归档/删除对等入口（唯一入口=桌面侧栏右键菜单；Sidebar <768px 退场后消失；SettingsTab onArchiveRole/onDeleteRole 形参自 16.x 起存量未接线）。
  evidence: 16.4 评审盲扫层（R10）：线框图四屏+设置 7 项+「⋯」菜单内容均为人工定稿（已进冻结块），未含该入口——intent 载体本身排除，故不判 bad_spec。但 web/移动宿主的能力对等是真实缺口：移动端能经 ButlerSettingsContent 恢复归档角色却不能归档/删除。 **收口（人类指令「移动端不能归档/删除角色 这个要修复」重新协商冻结边界）**：角色详情「⋯」菜单追加「归档 / 删除」（md:hidden 移动专属，桌面侧栏零变化），App 将既有 handleArchiveRole/handleDeleteRole 首次真正接到 RoleView；确认弹窗复用共享 Modal，语义照搬桌面 ConfirmDialog（删除需输入角色名、「至少保留一个角色」错误归一化）；新增 RoleHeader.archiveDelete.test.tsx 7 例 + web-mobile e2e 归档用例 1 例；线框图与 Spec Change Log 同步留痕。恢复路径不变（管家→设置→归档分区，移动端本就可达）。 **最终验证口径（如实）**：`npm run build` 零错误；前端单测 80 文件 935 例全绿（含新增 7 例）；归档 e2e 用例 4/4 全过（单跑、与 streaming 两连、两次全量中 web-mobile 均 11/11）。全量套件另见本文件末尾「套件级 setWindowSize 负载延迟面」延后项——与本次增量无关（控制实验已排除）。
- source_spec: `_bmad-output/implementation-artifacts/spec-16-4-mobile-web-form-and-pwa.md`
  summary: 全量 web e2e（test:web）在 VM 高负载下 setWindowSize 延迟生效，失败点随前序组合漂移（无确定性回归）。
  evidence: 16.4 D2 收口后共 9 次实测归纳。**现象**：`setWindowSize` 在 VM 高负载下可能延迟乃至长时间不生效，失败点随前序组合与负载漂移，无确定性回归——① 单跑 web-mobile（4 次）11/11 ✓；② web-mobile+web-streaming 两连 15/15 ✓；③ web-events+web-mobile 两连 11/11 ✓；④ web-reconnect+web-resident-loop+web-streaming 三连：挂 resident-loop 60s tick 用例而 streaming 4/4 ✓；⑤~⑨ 五次全量（`npm run test:web`）：红 5 次，挂点分别是 web-mobile before 钩子「视口 30/60 秒内未落定」（2 次）、web-streaming 375px 轮询「30 秒内未落定」+刷新恢复占位 45s 超时（3 次）；其中一次全量三 spec 全绿仅 streaming 2 例挂。**先例**：web-mobile 断点互斥用例注释既载「全量套件实测 500ms 竞态」（16.4 实现期）。**加固（用户 2026-09-25 授权「顺手加固其他 spec 的视口等待」）**：waitForViewport 区间轮询范式已推广到 web-streaming 375px 用例（原 pause(500ms) 无轮询）；web-mobile before/after/断点互斥三处预算 30/15/15s → 60/30/30s；web-streaming 375px 轮询 30s → 60s；web-streaming 补 after 钩子还原桌面宽度（视口纪律对齐 web-mobile）；web-mobile before 钩子 timeoutMsg 增加末次观测 innerWidth（失败自证，零断言影响）。**加固后**：两个受影响 spec 单跑均全绿（mobile 11/11、streaming 4/4），mobile before 钩子 60s 预算在全量中确实兜住过一次；但 streaming 的 30s 轮询在全量中出现过「60s 级仍不落定」的同类 stall（同次全量 mobile 60s 反而兜住）——说明该 stall 不是固定时长的延迟，而是浏览器进程在高负载末段的饥饿，视口轮询只能吸收「延迟」不能吸收「饿死」。**残留**：web-streaming「刷新恢复」用例的 45s 占位预算（Story 16.2 自留的环境抖动面，注释明载「跑三连套件偶发渲染停摆 15~30s」）在今日 5-spec 全量下已不够——非视口问题，超出本次授权范围，未动。 settle 条件：全量套件迁 nightly/CI（17.3 评审 B14 同源缺口）后在无人值守低负载环境观察；或对「resize stall」做 chromedriver 层面取证（如全量复现时 dump windowRect/handle 计数）。
- source_spec: `_bmad-output/implementation-artifacts/spec-16-4-mobile-web-form-and-pwa.md`
