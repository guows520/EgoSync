//! chat 域命令体（Story 15.4 自壳 `commands/chat.rs` 平移，业务逻辑/锁结构
//! 零改动；State/AppHandle 取值改 `&EngineCtx`，`<R: Runtime>` 泛型随 ctx 化
//! 自然退役，AppHandle 路径改 ctx 注入路径；chat_pick_working_directory 为
//! desktop-only 留壳不迁；内联测试随迁）。
//!
//! 六组会话状态类型自引擎 registry 回引（引擎内 `crate::commands::chat::{...}`
//! 路径语义与壳侧保持一致）。
...
pub async fn chat_send_message(
    ...
    let (main_pool, conv_pool, registry, agent_config): (
        &DbPool,
        &ConversationsPool,
        &Arc<ChatSessionRegistry>,
        &AgentConfigService,
    ) = (&ctx.pool, &ctx.conv_pool, &ctx.registry, &ctx.agent_config);
    ...
    // 覆盖，无需重复 emit。
    emit_chat_event(&*ctx.bus, crate::events::MESSAGE_SAVED_EVENT, &user_msg);
    ...
    let conv_pool_for_stream = conv_pool.clone();
    let conv_pool_for_memory = conv_pool.clone();
    let main_pool_for_stream = main_pool.clone();
    let main_pool_for_memory = main_pool.clone();
    // Story 15.4：AppHandle state 取值改 ctx 字段（同一实例克隆）
    let agent_bridge_clone = ctx.agent_bridge.clone();
    let event_router_clone = ctx.event_router.clone();
    let delegate_bridge_clone = ctx.delegate_bridge.clone();
    let streaming_state_clone = registry.streaming_state.0.clone();
    let registry_clone = registry.clone();
    let event_bus_clone = ctx.bus.clone();
    // Story 15.3：run_stream 宿主能力接缝注入（原 AppHandle 内部 try_state 取用）
    let agent_config_clone = agent_config.clone();
    let sidecar_clone = ctx.sidecar.clone();
    // Story 15.4：原壳侧即席 KeyringSecretStore 改经 ctx 注入的 SecretStore
    //（桌面壳注入同款 keyring 实现，值不变）
    let secret_for_stream: Arc<dyn SecretStore> = ctx.secrets.clone();
    // Story 15.3：默认 project_dir 由壳解析（原 engine 内 app_data_dir 逻辑迁壳侧）
    // Story 15.4：改 ctx 注入路径（桌面 = app_data_dir 派生 workspace）
    let project_dir_for_stream = {
        let workspace = ctx.opencode_workspace.clone();
    ...
    tokio::spawn(async move {
        ...
        let result = crate::services::agent_engine::run_stream(
...
pub async fn chat_get_history(
    ...
    conversations::list_messages(&ctx.conv_pool, &conversation_id).await
}
...
pub async fn chat_get_message_process_events(
    ...
    conversations::list_message_process_events(&ctx.conv_pool, &message_id).await
}
...
pub async fn chat_get_conversation(
    ...
    conversations::get_conversation(&ctx.conv_pool, &conversation_id).await
}

pub async fn chat_get_butler_conversation(ctx: &EngineCtx) -> Result<Conversation, AppError> {
    conversations::get_or_create_butler_conversation(&ctx.conv_pool).await
}
...
pub async fn chat_get_role_conversation(
    ...
    conversations::get_or_create_conversation_by_role(&ctx.conv_pool, &role_id).await
}
...
pub async fn chat_list_conversations(
    ...
    match role_id {
        Some(id) => conversations::list_conversations_by_role(&ctx.conv_pool, &id).await,
        None => conversations::list_butler_conversations(&ctx.conv_pool).await,
    }
}

pub async fn chat_stop_streaming(ctx: &EngineCtx, conversation_id: String) -> Result<(), AppError> {
    let registry: &Arc<ChatSessionRegistry> = &ctx.registry;
    ...
    if let Some(session_id) = session_id {
        let agent_bridge = &ctx.agent_bridge;
...
pub async fn chat_delete_conversation(ctx: &EngineCtx, conversation_id: String) -> Result<(), AppError> {
    let (registry, conv_pool): (&Arc<ChatSessionRegistry>, &ConversationsPool) =
        (&ctx.registry, &ctx.conv_pool);
...
pub async fn chat_new_conversation(
    ...
    let (main_pool, conv_pool, registry): (
        &DbPool,
        &ConversationsPool,
        &Arc<ChatSessionRegistry>,
    ) = (&ctx.pool, &ctx.conv_pool, &ctx.registry);
    if let Some(old_id) = old_conversation_id {
        ...
        if has_messages && !has_title {
            ...
            let main_pool_clone = main_pool.clone();
            let title_old_id = old_id.clone();
            let title_bus = ctx.bus.clone();
            let title_secrets = ctx.secrets.clone();
            tokio::spawn(async move {
...
async fn generate_title(
    ...
    // Story 15.4：密钥改经注入的 SecretStore（原壳侧 KeyringSecretStore 即席构造）。
    let provider =
        crate::services::llm_config::resolve_default_provider(&main_pool, secrets.as_ref())
            .await?;
