...
桌面应用支持与手机伴侣 App（`companion-android/`）在局域网内建立加密连接：

- **配对方式**：在桌面「全局设置 → 手机伴侣」中生成配对二维码，手机扫码即完成绑定。二维码单次有效（5 分钟内使用），重新生成会使旧码立即失效。
- **服务发现**：桌面通过 mDNS/NSD 广播服务 `_egosync._tcp`（实例名 `EgoSync-<relay_id 前 8 位>`）——配对窗口打开期间（等待扫码时）与配对过至少一台设备后均持续广播，手机在同一局域网内可自动发现桌面，免扫码重连。
...
- **当前限制**：中继服务（跨局域网连接）暂未部署，手机需与电脑处于同一局域网。
