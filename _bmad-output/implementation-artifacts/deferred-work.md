...
- source_spec: `_bmad-output/implementation-artifacts/spec-16-4-mobile-web-form-and-pwa.md`
  ...
  evidence: 16.4 D2 收口后 6 次实测归纳：① 单跑 web-mobile 11/11 ✓；② web-mobile+web-streaming 两连 15/15 ✓；③ web-reconnect+web-resident-loop+web-streaming 三连时挂的是 resident-loop 60s tick 用例而 streaming 4/4 ✓；④⑤ 两次全量均为 web-streaming 2 例挂（刷新恢复占位 45s 超时；375px 溢出名单含通知面板 left=800~1180 = 视口未缩到位）；⑥ 第三次全量挂的是 web-mobile before 钩子「视口 30 秒内未落到 320～430px」（钩子先于任何用例执行，直接排除 D2 增量嫌疑），streaming 反 4/4 全绿。先例：web-mobile 断点互斥用例注释既载「全量套件实测 500ms 竞态」（16.4 实现期），web-mobile 已用 waitForViewport 区间轮询加固（本次 30s 预算仍被打穿）；web-streaming 375px 用例仅 pause(500ms) 无轮询（Story 16.2 遗留）。按 AGENTS.md「连续失败 3 次」已停止重试并向用户报备。 settle 条件：把 waitForViewport 区间轮询范式推广到 web-streaming/web-events/web-resident-loop 的尺寸切换点（仅加固等待、不动断言），或全量套件进 nightly workflow（呼应 17.3 评审 B14 的同源缺口）后观察负载面。
- source_spec: `_bmad-output/implementation-artifacts/spec-16-4-mobile-web-form-and-pwa.md`
