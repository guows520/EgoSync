...
//!
//! desktop-only 名单（12 条）与 perf-test 门控名单（2 条）在此落死；
...
/// desktop-only 命令（12 条）：仅在桌面壳注册，server 物理不路由。
///
...
mod tests {
    ...
    #[test]
    fn desktop_only_list_pins_twelve_members() {
        assert_eq!(DESKTOP_ONLY_COMMANDS.len(), 12, "desktop-only 名单必须恰 12 条");
        for name in [
            "chat_pick_working_directory",
            "skill_pick_custom_directory",
            "pick_import_file",
